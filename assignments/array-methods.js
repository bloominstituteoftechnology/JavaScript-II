...
let fullName = [];
runners.map(function(___, number, arr){
    fullName.push(`${arr[number].first_name} ${arr[number].last_name}`)
})
...
let allCaps = [];
runners.map(function(__,number, arr){
    allCaps.push(arr[number].first_name.toUpperCase())
})
console.log(allCaps); 
...
let largeShirts = runners.filter(function(arrayItem){
    return arrayItem.shirt_size === 'L';
})
...
let ticketPriceTotal = runners.reduce(function (donantion, currentItem){
