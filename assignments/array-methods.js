...
const ticketPriceTotal = runners.reduce(function(totalDonation, item) {
    return totalDonation + item.donation;
}, 0);
...
// Problem 1
// Want to email everyone that donated more than $200. Get an array with the information for the people that donated more than $200.
const bigDonors = runners.filter((data) => {
...
let NameCompany = [];
runners.forEach(function(item) {
    NameCompany.push(`${item.first_name} ${item.last_name}: ${item.company_name}`);
});

console.log(NameCompany);

// Problem 3
// Decided to require a minimum donation of $100. We reached out to everyone that initially donated less than $100 and got them to donate the minimum of $100.
// Change the donation amount to $100 for everyone that had not donated over $100.
runners.forEach(function(item) {
