...
let largeShirts = [];
largeShirts = runners.filter((obj)    =>  {
...
ticketPriceTotal = runners.reduce((acc, obj)  =>  {
    acc += obj.donation
    return acc;
}, 0)
...
// Problem 1
let largeDonators = []
largeDonators = runners.filter(obj  =>  {
    return obj.donation > 99;
});
console.log(largeDonators);

// Problem 2
let companies = [];
companies = runners.map((obj)   =>  {
...
// Problem 3
let runnersAndEmails = []
runnersAndEmails = runners.map((obj)    =>  {
    return {first_name: obj.first_name,
            email: obj.email}
});
console.log(runnersAndEmails)
