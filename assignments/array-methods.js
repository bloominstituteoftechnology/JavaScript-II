...
let fullName = [];
    runners.forEach(function(oranges) {
        let name = (`${oranges.first_name} ${oranges.last_name}`);
        fullName.push(name)
...
let allCaps = [];
const bigLetters = (str) => {
    return str.toUpperCase();
}
allCaps = fullName.map(bigLetters)
console.log(allCaps); 
...
let largeShirts = [];
const sizeLarge = (object) => {
    return object.shirt_size === "L"
}
largeShirts = runners.filter(sizeLarge)
