...
allCaps = runners.map(function(item) {
    return item.first_name.toLocaleUpperCase();
})
console.log(allCaps); 
...
// Problem 1
let smallShirtsTotal = [];
smallShirtsTotal = runners.reduce(function(theReducer, item) {
    return theReducer += item.shirt_size === 'S';
}, 0);
console.log(smallShirtsTotal);

// Problem 2
let bigDonation = [];
bigDonation = runners.filter(function(item) {
...
let companyName = [];
runners.forEach(function(item) {
    companyName.push('Name: ' + item.first_name + ' ' + item.last_name + ', ' + 'Company: ' + item.company_name);
})
