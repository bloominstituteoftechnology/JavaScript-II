...
fullName.push(arrItem.first_name + ' ' + arrItem.last_name);
});
...
let allCaps = [];
fullName.map(function(arrItem){
allCaps.push(arrItem.toUpperCase())
})
...
let largeShirts = [];
const result = runners.filter(arrItem => arrItem['shirt_size'] === 'L');
...
let eachDonation = []
runners.forEach(function (arrItem) {
    eachDonation.push(arrItem.donation);
    });
let donationTotal = eachDonation.reduce((acc, val) => {
    return acc + val;
})
