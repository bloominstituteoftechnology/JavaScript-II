...
const largeShirts = runners.filter(item => item.shirt_size === "L")
...
const ticketPriceTotal = runners.reduce((acc, item) => {return acc + item.donation}, 0);
...
// Problem 1
// We need to know which runners are in school, to give them a discount.
const discountHolders = runners.filter(runner => runner.email.includes(".edu"));
console.log(discountHolders); 


// let str = "example@example.edu";
...
// Problem 2
// We need to organize our runners to distribute their shirts so we need to find out which names start with S.
const namesWithS = [];

 runners.forEach(runner => {
    if (runner.first_name.split("")[0] === "S"){
    namesWithS.push(runner.first_name)};
});
