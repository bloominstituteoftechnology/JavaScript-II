...
const largeShirts = runners.filter( function(info) {
...
const ticketPriceTotal = runners.reduce( function(theReducer, runner) {
    return theReducer += runner.donation;}, 0);
...
// Problem 1
// We've decided to put the names on the back of the shirts. We need the last names and shirt sizes for all runners.
let shirts = [];
runners.forEach( function(runner) {
    shirts.push(runner["first_name"] + runner["last_name"] + " has a shirt size of " + runner["shirt_size"])
});
console.log(shirts);

// Problem 2
// We screwed up, the full names won't fit on the back. We can only fit the last names in lower case.
const lower = runners.map( function(item) {
    return item.last_name.toLowerCase();
})
console.log(lower); 

// Problem 3
// We are going to give a gift to all runners that donated at least $250, return the information of the runners that did.
const bigSpenders = runners.filter( function(info) {
    return info.donation >= 250;
});
