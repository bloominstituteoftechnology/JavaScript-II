...
runners.forEach(function (currentValue) {
    fullName = currentValue.first_name + " " + currentValue.last_name;
...
const newRunners =
runners.map(function(currentValuez){
    return currentValuez.first_name.toUpperCase();
})
...
let largeShirts = [];
const newRunnersSize =
runners.filter(currentV => {
    return currentV.shirt_size === "L"
})
console.log(newRunnersSize)


// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
const totalDonation = runners.reduce(function(ticketPriceTotal, tDonation){
    return ticketPriceTotal + tDonation.donation; 
}, 0);
