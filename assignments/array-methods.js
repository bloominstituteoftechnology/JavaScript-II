...
runners.forEach((elem) => {
    fullName.push({first_name: elem.first_name, last_name: elem.last_name}); 
});
...
ticketPriceTotal = runners.reduce((sum, item) => {
...
// Problem 1
// The event director has decided that he likes the name Rectangle. 
// so he now wants all of the runners running bibs to have rectangle 
// instead of their first name and he is nice enough to let them keep 
// thier last names. make a new array of first and last names where 
// the first name of everyone shall now be Rectangle! 
// The director may have gone a bit mad at this point.
let theRectangles = [];
runners.forEach((elem) => {
    theRectangles.push({first_name: "Rectangle", last_name: elem.last_name}); 
});
console.log(theRectangles);

// Problem 2
// The director has decided on a minimum donation for the run 
// should be 100 so he wants anyone who has paid less to be 
// labeled unimportant and have a lower case name and the word 
// unimportant as their first name 
// (you are not allowed to use filter() for this problem)
let unimportantRunners = [];

runners.forEach((elem) => {
    if(elem.donation < 100) {unimportantRunners.push({first_name: "unimportant", last_name: elem.last_name.toLowerCase()}); }
});

console.log(unimportantRunners);

// Problem 3
// The director is feeling generous and has decided that
// the people who have donated over 120 should get a reward and 
// be sent on a flight around the world. he needs you to 
// get a list of the important donators for the logistics team
// use filter and make a new array to hold the important donators. 
// remember to console.log the resulting data

let importantRunners = [];

importantRunners = runners.filter((data) =>{
    return data.donation > 120;
    });

console.log(importantRunners);