...
let fullName = [];
runners.forEach(function(xrunners){
fullName.push(xrunners.first_name + " " + xrunners.last_name);
});
...
// let allCaps = [];
let allCaps = runners.map(function(xrunners){
   return xrunners.first_name.toUpperCase();
});
...
let largeShirts = runners.filter(function(size){
...
let ticketPriceTotal = runners.reduce((total, ammount, index, array) => {
   return total + ammount.donation;
}, 0);
...
// Problem 1 a few companies want to group together
let company1 = [];
let company2 = [];
let company3 = [];
let companyGroup = runners.forEach((group) => {
    if (group.company_name == "Skinix") {
        company1.push(group.first_name + " " + group.last_name);
    }
    else if (group.company_name == "Wordtune") {
        company2.push(group.first_name + " " + group.last_name);
    }
    else if (group.company_name == "Gigashots") {
        company3.push(group.first_name + " " + group.last_name);
    }
...
// Problem 2 director wants to know all the names and emails so they can remind the runners of the date
email = [];
runners.forEach(function(xrunners){
    email.push( `[Email:${xrunners.email}] [Name: ${xrunners.first_name}  ${xrunners.last_name}]`);
    });
    console.log(email);

// Problem 3 biggest donators get to the front of the line

let bigDonors = []; 
runners.filter(function(money){
    if (money.donation > 100) {
        bigDonors.sort().reverse().push(`[Donation:${money.donation}] [Name:${money.first_name} ${money.last_name}]`);
    }
