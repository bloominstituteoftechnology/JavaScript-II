...
let ticketPriceTotal = runners.reduce((donations,runner) => {
    return donations += runner.donation;
},0);
...
// Problem 1: filter by the runner that did the most donations
let donations = runners.filter(runner => {
    return runner.donation > 200;
});
console.log(donations); 

// Problem 2: sort by shirt size
// const shirtSize = runners.map(runner => runner.shirt_size).sort();
