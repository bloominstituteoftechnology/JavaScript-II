...
ticketPriceTotal.push(runners.reduce((acc, val) => {
...
// Problem 1 the director needs the first and last names capitalized  
let FirstLastCaps = [];
FirstLastCaps = runners.map((x) => {
    return `${x.first_name.toUpperCase()} ${x.last_name.toUpperCase()}`
})
console.log(FirstLastCaps)
// Problem 2 the director wants a list of the companies who donated more than 150$.
let moreThan150 = [];
moreThan150 = runners.filter((x) => {
    return x.donation > 149;
})

console.log(moreThan150)
// Problem 3 The director wants to know which companies to hitup for more money. Return a list of all the comapnies who donated less than 50$.
let lessThan50 = [];
lessThan50 = runners.filter((x) => {
