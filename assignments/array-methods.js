...
people = runners.filter(item => item.shirt_size === "L");

people.forEach(item =>
  largeShirts.push(
    `${item.first_name} ${item.last_name} - size: ${item.shirt_size}`
  )
);
...
ticketPriceTotal = runners.reduce((amt, person) => amt + person.donation, 0);
...
runners.forEach(runner =>
  emailAddress.push(`${runner.first_name} ${runner.last_name}: ${runner.email}`)
);
...
donors = runners.filter(item => item.donation >= 200);

donors.forEach(item =>
  bigDonors.push(`${item.first_name} ${item.last_name}: $${item.donation}`)
);

console.log(bigDonors);

// Problem 3
let runnerCompanies = runners.map(runner => runner.company_name);
