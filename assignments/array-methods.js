...
// Problem 1
// The event director needs a list of companies of the runners sorted alphabetically.
let companyList = [];
companyList= runners.map(runner => runner.company_name).sort();
console.log(companyList);

// Problem 2
// The event director needs a list of runners that donated over 100 dollars to the run.
let highDonators = [];
highDonators = runners.filter(runner => runner.donation >= 100);
console.log(highDonators);

// Problem 3
// The event director wants a list of the shirt sizes needed for all runners where each shirt size has the number of runners that requested that size.
