...
let ticketPriceTotal = runners.reduce((blarg, runners) => blarg + runners.donation, 0);
...
// Problem 1: Get a list of all the email adresses of anyone with a large shirt.

let emailAdd = []
largeShirts.forEach((largeShirts) => emailAdd.push `${largeShirts.email}`)
console.log(emailAdd);

// Problem 2: Find the company name of anyone who donated more than 200.

let bigDon = [];
runners.forEach((runners) => {if (runners.donation >= 200) { bigDon.push `${runners.company_name}`}})

console.log(bigDon)

// Problem 3: for all runners who's names start with M.


let mLowCase = runners.filter ((runners) => {
    return runners.first_name[0].toLowerCase() === 'm';
  });

console.log(mLowCase);