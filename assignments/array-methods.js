...
let allCaps = [];
const newCapNames = runners.map(function(item) {
...
const shirtSizeFilter = runners.filter((data) => {
  if (data["shirt_size"] === "L") {
    return data["shirt_size"] = "L";
  }
})
console.log(shirtSizeFilter);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result

const ticketPriceTotal = runners.reduce((sum, order) =>{
  return sum += order.donation;
}, 0)
...
// The event director needs all participants' full names and donation so he can personally send them thank-you emails. Combine the email and donation amount into a new array called thankYouInformation
let thankYouInformation = [];
runners.forEach(function(element) {
  thankYouInformation.push(element["first_name"] + "," + element["last_name"] + ", donated " + "$" + element["donation"] );
})

console.log(thankYouInformation);
...
//The event director, needs the company names of each contestant capitalized to look "professional", as he calls it. Make sure the company names are in alphabetical order, and next to each company name, the corresponding person's name.

let companyTeeShirts = [];
runners.map(function(element) {
  companyTeeShirts.push(element["company_name"].toUpperCase() + "->" + element["first_name"] + "-" + element["last_name"]);
})

console.log(companyTeeShirts.sort());
...
// Problem 3
// The event director got word that the CEO at Skinix wants to see which of his employees skipped work to do the 5k. You know you shouldn't, but its also your second day in the company.

let workSkippers = runners.filter((data) => {
  if (data["company_name"] === "Skinix") {
    return data["company_name"] = "Skinix";
  }
})

console.log(workSkippers);
