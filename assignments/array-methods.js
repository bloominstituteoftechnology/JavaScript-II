...
runners.map(firstNames => firstNamesAllCaps.push(firstNames.first_name.toUpperCase()));
...
let runnersLargeSizeShirt = runners.filter(lrgShirts => lrgShirts.shirt_size === "L");
...
let ticketPriceTotal = runners.reduce((acc, donated) => acc += donated.donation,0);
...
// Problem 1
// We lost all the shirt information and need to email all the runners regarding their size. 

let emails = [];

runners.forEach(runnerEmail => emails.push(runnerEmail.email));
...
let avgDonation = runners.reduce((acc,donated) => acc+=donated.donation,0);
numOfRunners = runners.length;

console.log("$"+avgDonation/numOfRunners);

// Problem 3
// The couch has lost his mind.  He now believes that the letter L means loser and wants everyone whose name starts with an "L" off the team.  He needs a list.

let losers = [];

runners.filter(function(gottaGo){
  if(gottaGo.last_name[0] === "L"){
    losers.push(gottaGo.first_name+" "+gottaGo.last_name);
  }
})
