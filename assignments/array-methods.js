...
runners.forEach(forEachCallback);

function forEachCallback(runner) {
...
const allCaps = runners.map(function(shirt){
 return shirt.first_name.toUpperCase();
})
  console.log(allCaps); 
...
const largeShirts = runners.filter( function(size){
...
let ticketPriceTotal = runners.reduce(price, 0);
   function price(accumulator, currentRunner){
      return accumulator + currentRunner.donation;
   }
