...
let ticketPriceTotal =
    runners.reduce(function (acc, value) {
...
// Problem 1
//The secretary need everyone's email address so that she can send out any last minute details
let runEmail = [];
runners.forEach(function (contact) {
    runEmail.push(`${contact.email}`)
});
console.log(runEmail);

// Problem 2
// In our press release, we want to list all the company names that the runners represent
let repWork = [];
runners.forEach(function (company) {
    repWork.push(`${company.company_name}`)
});
console.log(repWork);

// Problem 3
// The tshirt company, did not make aby small shirts, made all sizes from medium up. Need to get a list of all those who requesteda small shirt.

let smallShirts =
    runners.filter(function (small) {
