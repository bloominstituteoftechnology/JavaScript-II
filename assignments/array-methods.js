...
// })
allCaps = fullName.map(el => el.toUpperCase());
console.log(allCaps); 
...
//   largeShirts.push()
// })
largeShirts = runners.filter(el => el.shirt_size == "L");
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketArr = runners.map(el => el.donation)
console.log(ticketArr);
ticketPriceTotal = (accumulator, current) => accumulator + current;
console.log(ticketArr.reduce(ticketPriceTotal));

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to solve 3 unique problems using one or many of the array methods listed above.

// Problem 1: Show a list of all the companies being represented by the runners.
let corpList = [];
corpList = runners.map(el => el.company_name);
console.log("#1");
console.log(corpList.sort());
...
runners.forEach(function(el){
  if (el.donation <= 100){
    donorLevels["bronze"] += 1;
  } else if (el.donation <= 200 && el.donation > 100){
    donorLevels["silver"] += 1;
  } else if (el.donation <= 300 && el.donation > 200){
    donorLevels["gold"] += 1;
  } else if (el.donation > 300){
    donorLevels["platinum"] += 1;
  }
})
console.log(donorLevels);

// Problem 3: Any duplicated first names?
let nameBadgeDuplicates = [];
let nameCounts = {};
let firstNames = runners.map(el => el.first_name);
console.log(firstNames);
firstNames.forEach(function(el){
  console.log(el);
  if (nameCounts[el] != true){
    nameCounts[el] = 1;
  } else if (nameCounts[el] == true){
    nameCounts[el] += 1;
  }
})
