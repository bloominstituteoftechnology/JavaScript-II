...
let largeShirts = [];
largeShirts = runners.filter(people => people.shirt_size == "L");
...
let ticketPriceTotal = runners.reduce(
    (donationBin, person) => donationBin + person.donation, 0);
...
// Problem 1 The organizers need to send out e-mails to each runner to give them their booth information. Return an array of objects with the first name and e-mail of each runner.

let compArr = runners.map(person => ({ name: person.first_name, email: person.email }));

console.log(compArr);

// Problem 2 - Anybody with a donation of over $100 gets a special prize. Return an array of runners with donations over $100.

let topDonations = [];
topDonations = runners.filter(people => people.donation >= 100);

console.log(topDonations);



// Problem 3 - On your marks, get set, go!


let shuffled = runners
