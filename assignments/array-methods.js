...
//     fullName.push(runners[i].first_name + " " + runners[i].last_name); //for loop that accomplishs the task.
// }

runners.forEach((a)=> {
    fullName.push(`${a.first_name} ${a.last_name}`);
});
...
//phrasing is hard to interpret
let largeShirts = runners.filter((a) => {
     return a.shirt_size === "L";
});
...
// console.log(runners[3].donation, runners[4].donation, runners[24].shirt_size);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 (Sort Email's alphabetically)
...
console.log(ballerDonations);

// Problem 3 (Change All Company Names to google )

let googleTakeover = runners.map((a) => {
    return a.company_name = 'Google';
});

console.log(googleTakeover);