...
let allCaps = runners.map(item => item.first_name.toUpperCase());
// console.log(allCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.
// Get a list of runners with large sized shirts so they can choose a
// different size. Return an array named largeShirts that contains information
...
let ticketPriceTotal = runners.reduce(
    (total, item) => total + item.donation,
    0
);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I
// want you to think of potential problems you could solve given the data set
// and the 5k fun run theme.  Try to create and then solve 3 unique problems
// using one or many of the array methods listed above.

// seperate runners into bronze, silver, and gold categories based on how many donations they got and then list out their names.
let bronze = runners
    .filter(item => item.donation >= 50 && item.donation < 100)
    .map(item => `${item.first_name} ${item.last_name}`);
// console.log(bronze);
let silver = runners
    .filter(item => item.donation >= 100 && item.donation < 200)
    .map(item => `${item.first_name} ${item.last_name}`);
// console.log(silver);
let gold = runners
