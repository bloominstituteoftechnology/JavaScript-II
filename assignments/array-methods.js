...
runners.forEach( run => {
  let addName = run.first_name + " " + run.last_name;
  fullName.push(addName);
})
console.log(fullName);
...
allCaps = runners.map( cap => cap.first_name.toUpperCase());
console.log(allCaps);
...
largeShirts = runners.filter( lshirt => lshirt.shirt_size === "L");
console.log(largeShirts);
...
donation_value = runners.reduce((accumulator, currentValue) => accumulator + currentValue.donation, 0);
ticketPriceTotal.push(donation_value);
