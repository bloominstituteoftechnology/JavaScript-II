...
let fullName = [];
runners.forEach((monkey) =>{
  return fullName.push(`${monkey.first_name} ${monkey.last_name}`)
});
...
allCaps = runners.map((monkey) => {
  return monkey.first_name.toUpperCase();
  });
...
largeShirts = runners.filter(placeholder => {
  return placeholder.shirt_size === 'L'
})
...
// Problem 1 -console log the first and last names of all //participants with their companies

let directory = [];

runners.forEach((elem) => {
  return directory.push(`${elem.first_name} ${elem.last_name} is from the company ${elem.company_name}`);
})

console.log(directory);

// Problem 2 - print out an array of the names of all donors who //donated over $100 using a filter method

let bigSpender = [];

bigSpender = runners.filter(elem =>{
  return elem.donation >= 100;
});

console.log(bigSpender);

//Have John help with sorting out the names and not just returning the entire array


// Problem 3-Use map to "send" a thank you to each donor
