...
let allCaps = [];
allCaps = runners.map(function(runner){return runner.first_name.toUpperCase()});
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// // The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
largeShirts = runners.filter(function(runner){return runner["shirt_size"]==="L"});
console.log(largeShirts);

// // ==== Challenge 4: Use .reduce() ====
// // The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal = runners.reduce(function(a, b){return (a += b.donation)}, 0);
...
// // // Problem 1
let mailingList = [];
runners.forEach(function(runner){ mailingList.push(runner.first_name + " " + runner.last_name + " <" + runner.email + ">")});
console.log(mailingList);


// // Problem 2
let donationOrder = runners;
donationOrder = runners.slice(0).sort(function(a, b){ return a.donation - b.donation});
...
// // Problem 3
runners.forEach(function(runner){ return runner.id = Math.floor(Math.random() * Math.floor(runners.length - 1))});
console.log(runners);