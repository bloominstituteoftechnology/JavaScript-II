// A local community center is holding a fund raising 5k fun run and has invited 50 small businesses to make a small donation on 
// their behalf for some much needed updates to their facilities.  Each business has assigned a representative to attend the event 
// along with a small donation.

// Scroll to the bottom of the list to use some advanced array methods to help the event director gather some information from the 
// businesses.
...
// The event director needs both the first and last names of each runner for their running bibs. Combine both the first and last 
// names and populate a new array called `fullNames`. This array will contain just strings.
let fullNames = []
...
const firstNamesAllCaps = runners.map(runner => runner.first_name.toUpperCase());
...
const runnersLargeSizeShirt = runners.filter(runner => runner.shirt_size === 'L');
...
const ticketPriceTotal = runners.reduce((total, current) => {
  if (isNaN(total)) total = current.donation; // first iteration of total is simply first object in runners array
...
//  given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.
