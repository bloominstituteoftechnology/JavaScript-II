...
// Problem 1
// Runners that work at Skinix still need to fill out their bios for the Fun Run program. Find all the runners that work at Skinix then put their emails into an array so they can be sent a reminder email.
const emails = runners
  .filter(runner => runner['company_name'] === 'Skinix')
  .map(runner => runner['email']);
console.log('Send a reminder email to: \n', emails);

// Problem 2
// The event committee wants to tier the donations for the program. Group the runners into donor categories based on their contribution (below) and print out the companies in each category
...
console.log('Our Gold company donors giving between $250-$300 are: \n', gold);
console.log(
  'Our Silver company donors giving between $150-$249 are: \n',
  silver
);
console.log('Our Bronze company donors giving between $1-$149 are: \n', bronze);
...
const entries = [];
runners
  .filter(runner => runner['donation'] >= 150)
  .forEach(runner =>
    entries.push(
      `${runner['first_name']} ${runner['last_name']} from ${
        runner['company_name']
      }`
    )
  );
...
console.log(`${winner} won an iPad (or something!)`);
