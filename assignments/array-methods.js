...
// Problem 1
// The officials have asked that every runner needs to fax their medical history before running the race. Alphabetize the list by last name, then log the matching name and email in a new array titled EmailList

EmailList = [];

runners.forEach(function(runners) {
    EmailList.push(runners.last_name + " Email: " + runners.email);
});

console.log(EmailList.sort());
*/
// Problem 2
// We want to list of all the sponsors who donated a high ammount. Make a list of each sponsor that made a donation of over 200.
SponsorList = [];

runners.forEach(function(runners){
    if (runners.donation > 200) {
        SponsorList.push(runners.company_name)
    }
});

console.log(SponsorList.sort());

// Problem 3
// The shirts used need to have an ID number and what sponsor they are running for. Create a new array for this information. 