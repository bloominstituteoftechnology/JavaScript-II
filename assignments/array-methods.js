...
// let ticketPriceTotal = runners.reduce((a, b) => ({ x: a.x + b.x }));
...
// Problem 1
// Find the 'Premium Doners' who's donation is above 100 and return their information in an arrsay of objects.

const primeDoners = runners.filter(runner => runner.donation >= 100);

console.log(primeDoners);

// Problem 2
// Find number of shirts for any given size

const getShirtSizeTotal = size =>
  `There are ${
    runners.filter(runner => runner.shirt_size === size).length
  } ${size} shirts.`;

console.log(getShirtSizeTotal("S"));
console.log(getShirtSizeTotal("M"));
console.log(getShirtSizeTotal("L"));
console.log(getShirtSizeTotal("XL"));

// Problem 3
// Get an array of each members email for your mailchip email list to easily send all the event infomation.

const emailList = runners.map(runner => runner.email);
