...
const capsFirst = (upperCaseNames) => {

  allCaps += `${upperCaseNames.first_name.toUpperCase()} ${' '}`;
};
runners.map(capsFirst);
...
let largeShirts = [];
const whosShirtIsThis = (shirts) => {
  if (shirts.shirt_size === "L") {
    largeShirts += `${shirts.first_name} ${ shirts.last_name} ${'Shirt Size:'} ${shirts.shirt_size} ${' '}`;
  }
};

runners.filter(whosShirtIsThis);
...
console.log(runners.reduce(totalDonations));

const total = ticketPriceTotal.reduce((total, sumDonations) => total + sumDonations);
