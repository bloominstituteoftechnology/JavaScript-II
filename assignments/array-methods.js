...
//     let newname = `${runners[i].first_name}  ${runners[i].last_name}`;
...
// }
allCaps = runners.map((data, index, arr) => {
...
ticketPriceTotal = runners.reduce((total, data, index, arr)=>{
    return total += data.donation;
}, 0);
console.log('******************');
...
console.log('******************');
 console.log("There are ", count, " companies.");
 console.log('******************');
...
// Problem 3
//gather all emails
const listOfEmails = runners.map((data, index, arr)=> {
    return data.email;
});
