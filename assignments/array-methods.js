...
//****** Problem 1*******: Event organizers need to know the amount of each shirt size to order.
let shirtAmt = {};
//get unique shirt sizes
let shirtSizes = new Set(runners.map(runner => runner.shirt_size));

//Log amount of shirt for each size into shirtAmt
shirtSizes.forEach(function(size) {
    let sizeAmt = runners.filter(function (runner){
        return runner.shirt_size === size;
    })
    shirtAmt[size] = sizeAmt.length; 
})
//Short Version
//shirtSizes.forEach(size => shirtAmt[size] = runners.filter(runner => runner.shirt_size === size).length); 
console.log(shirtAmt);

// ***** Problem 2 ***** Get a list of all unique companies participating the event using map + filter
let allCompanies = runners.map((runner) => runner.company_name);
let uniqueCompanies = allCompanies.filter((company,pos) => allCompanies.indexOf(company) === pos);
console.log(uniqueCompanies);

// ****Problem 3**** Donation per company
let donations = {};
uniqueCompanies.forEach(company => donations[company] = runners.filter(runner => runner.company_name===company).reduce((total, runner) => total += runner.donation, 0));
