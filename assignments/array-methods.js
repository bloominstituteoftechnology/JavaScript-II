...
ticketPriceTotal = runners.reduce((reducer, runner) =>  reducer += runner.donation, 0);
...
runners.map((runner) => {
    if (runner.donation >= 200) {
        VIP.push(`${runner.first_name} ${runner.last_name}`);
    } else if(runner.donation <= 100){
        lowBaller.push(`${runner.first_name} ${runner.last_name}`);
    }
});
...
let adults = [];
adults = runners.filter((runner) => runner.shirt_size != "XS" && runner.shirt_size != "S"); 
