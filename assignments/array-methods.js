...
let fullName = [runners.forEach(function(first_name, last_name) {
})];
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = [runners.map(function(first_name) {
...
let largeShirts = [runners.filter(function(shirt_size) {
    return shirt_size === "L";
})];
...
let ticketPriceTotal = [runners.reduce(function(donation, ) {
...
// Problem 1
// event director wants email and last name: 
let info = [funner.forEach(function(email, last_name) {    
})];
console.log(info);

// Problem 2
//the power hungry director was fired and now the new one wants all last names to be caps to match.
let finalAllCaps = [runners.map(function(last_name) {
    return last_name.toUpperCase()
})];
console.log(finalAllCaps); 

// Problem 3
// director needs a list of everyone named hank because he also is named hank and cant have anyoe stealing his thunder.
let hankHunt = [runners.filter(function(first_name) {
    return first_name === "Hank";
})];
console.log(hankHunt);