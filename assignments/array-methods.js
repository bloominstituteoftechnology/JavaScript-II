...
// Problem 1 - The race director needs to make banners with the names of the companies. Use .forEach to put the company names in a new array called companyName.
let companyName = [];
runners.forEach(function(element) {
    companyName.push(element.company_name);
});
console.log(companyName);

// Problem 2 - The race director needs a email list for sending out race photos. Create a new array that contains only the email addresses of the racers.
let addresses = [];
runners.forEach(function(element) {
    addresses.push(element.email);
});
console.log(addresses); 

// Problem 3 - Your crazy race director wants to let anyone who donated over $250 start the race ahead of everyone else. Create an array using .filter that gives the names of these individuals. 
let startPrice = runners.filter((data) => {
    return data.donation >= 250;
});
