...
const allCaps = fullName.map(name => name.toUpperCase())
console.log(allCaps)
...
// Problem 1
// return of array of all players alphebetized by last name with their shirt size
const sizes = runners
  .map(
    ({ first_name, last_name, shirt_size }) =>
      `${last_name} ${first_name}: ${shirt_size}`
  )
  .sort()
console.log(sizes)

// Problem 2
// reassign id's based on alphebetized last name
...
// Problem 3
// return an object containing number of shirts of each size
const shirts = runners.reduce(
  (totals, { shirt_size }) => ({
    ...totals,
    [shirt_size]: totals[shirt_size] + 1
  }),
...
// make sure that we accounted for all the shirts
console.log(
  Object.keys(shirts)
    .map(key => shirts[key])
