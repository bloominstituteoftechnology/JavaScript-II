...
runners.forEach(function(name) {
  let firstName = [];
   firstName.push(name.first_name);
  let lastName =  [];
  lastName.push(name.last_name);
  //  console.log(lastName);
  const combineNames = firstName + "" + lastName;
  fullName.push(combineNames);
  console.log(fullName);
});

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = runners.map(function(nameCap) {
  return nameCap.first_name.toUpperCase();
});
console.log(allCaps);
...
const ticketPriceTotal = runners.reduce((theReducer, item) =>{
...
// Problem 1
//Make a list of the companies participating in alphabetical order.
let runningCompanies = [];
for(let i=0; i< runners.length; i++) {
  runningCompanies.push(runners[i].company_name)
  }
console.log(runningCompanies.sort());

// Problem 2
//The Runners who doante over 100$ will be given a special prize, but who are they?
let prizeWinners = [];
for(let i=0; i<runners.length; i++) {
if(runners[i].donation >= 100) {
  prizeWinners.push(runners[i].last_name)
  }
  }
...
// Last minute notes about what is appropriate to wear running need to to be sent out, immediately. We need those emails!
runners.forEach(function(importantNote) {
  console.log(importantNote.email);
});