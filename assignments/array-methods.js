...
const allCaps = runners.map( folks => folks.first_name.toUpperCase());
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
// let largeShirts = runners.filter(folk => folk.shirt_size === "L");

// console.log(largeShirts);

let largeShirts = runners.filter((backorder) => {
    return backorder.shirt_size==='L'
  });
  console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((total, folk) => total + folk.donation, 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1- create an array of strings that tell us each runner's email

const emailList = runners.map(folk => folk.first_name + " " + folk.last_name +" [" + folk.email + "]");
console.log(emailList)

// Problem 2 - Return an array of all runners with over $200 in donations
bigDonor = runners.filter( folk => folk.donation > 200);
console.log(bigDonor);

// Problem 3- Rank the runners by the size of their donations.

const sortDonations = runners.sort((a, b) => b.donation - a.donation);
console.log(sortDonations)