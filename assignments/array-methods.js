...
let firstNamesAllCaps = runners.map((firstname) => {
    return { 'first_name': firstname.first_name.toUpperCase() };
});
...
let ticketPriceTotal = runners.reduce((donations, taxes) => {
    return donations += taxes.donation
}, 0)
...
runners.forEach((lastname) => {
    lastname.last_name;
    newlastname.push(lastname.last_name)
});
console.log(newlastname)
    // Problem 2 - Getting the students that were cheap and donated less than $50.

let cheaprunners = runners.filter((cheap) => {
    return cheap.donation < 50
});
