...
ticketPriceTotal = runners.reduce((acc,runners) => acc + runners.donation,0);
...
// Problem 1
// create an array of  all the participants that donated more than 100 dollars
let highDonation = [];
runners.forEach((element) =>{
  if (element.donation > 100){
    highDonation.push(`${element.first_name} ${element.last_name} donated ${element.donation} dollars`)
  }
})
console.log (highDonation);

// Problem 2
//  subtract 10 % from all donations and log the total deduction and the percentage taken.
const cookedBooks = runners.reduce((total, dons)=> total + dons.donation ,0);

console.log(cookedBooks);
console.log (cookedBooks - (cookedBooks * .1));
console.log (cookedBooks * .1);
// Problem 3
// create an array of participant email addresses for spam
runners.forEach(element => console.log (element.email));
