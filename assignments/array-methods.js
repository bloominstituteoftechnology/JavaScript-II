...
let largeShirts = runners.filter(function(shirts){
...
//arrow functions
let filter_shirts = runners.filter(
    shirts => shirts.shirt_size == 'L'
    ).map(
        runner => `${runner.first_name} ${runner.last_name} wears a size: ${runner.shirt_size}`
    );
console.log(filter_shirts);
...
//arrow
let dontationTotal = runners.reduce((total, runner) => total + runner.donation, 0);
