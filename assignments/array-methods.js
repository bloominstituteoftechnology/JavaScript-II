...
// Problem 1
// find all companies with more than one runner
let companies = []
companies = runners.reduce((acc, runner) => {
  acc[runner.company_name] ? acc[runner.company_name]++ : acc[runner.company_name] = 1;
  return acc;
}, {})
companies = Object.entries(companies).filter(([_, num]) => num > 1);
console.log(companies);

// Problem 2
// runners with above average donations
let aboveAvgDonation = []
aboveAvgDonation = runners.filter(runner => runner.donation > (ticketPriceTotal / runners.length));
console.log(aboveAvgDonation);

// Problem 3
// runners who are students
let students = []
students = runners.filter(runner => runner.email.match(/\.edu$/g));
