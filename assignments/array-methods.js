...
let allCaps = runners.map(names => (names.first_name.toUpperCase()));
console.log(allCaps);
...
let ticketPriceTotal = runners.reduce(function(totalDonations,item) {
    return totalDonations + item.donation;
},0);
...
//Arrow function
// let bigDonors = runners.filter(runner => runner.donation > 200);
console.log(bigDonors);

// Problem 2
//Total donations for bigDonors
let totalBigDonors = bigDonors.reduce(function(total, runner) {
    return total + runner.donation;
},0);

console.log(totalBigDonors);

// Problem 3
//Total donations for $200 or below
let smallDonorTotal = runners.filter(runner => runner.donation <= 200).reduce(function(total, runner) {
   return total + runner.donation; 
},0);
console.log(smallDonorTotal);