...
runners.forEach(x =>  console.log(`${x['first_name']} ${x['last_name']}`));
...
// Problem 1 Student Discount! Every runner with a .edu email address will receive a discount on concessions. Grab their email addresses to send them the coupon.
let students = runners.filter(x => /edu$/.test(x.email))
console.log(students.map(x => x.email))

// Problem 2 Disqualified! Skinix has been busted for breaking child labor laws. Given the fact that most of their recruitment took place at the community center in question, the organizers found it would be best to distance themselves from Skinix and their employees. Remove all Skinix employees from the list.
let filteredRunners = runners.filter(x => x.company_name !== 'Skinix')

console.log(filteredRunners)


// Problem 3 Limited Match! An eccentric billionaire has decided to help out the Community Center. She has offered to match every donation, as long as that donated amount is divisible by three. How much money will she be donating?
let eccentricDonation = runners.filter(x => !(x.donation % 3)).reduce((a,b) => a + b.donation, 0 )
console.log(eccentricDonation)
