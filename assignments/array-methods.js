...
let fullName = [];
runners.forEach(rep => {
  fullName.push(`${rep.first_name} ${rep.last_name}`);
})
...
let allCaps = [];
runners.map(rep => {
  allCaps.push(rep.first_name.toUpperCase())
})
console.log(allCaps);
...
let largeShirts = [];
runners.filter(reps => {
  // if (reps.shirt_size === 'L' || reps.shirt_size === 'XL' || reps.shirt_size === '2XL' || reps.shirt_size === '3XL') {
  //   return largeShirts.push(reps.shirt_size);
  // }
  switch(reps.shirt_size) {
    case 'L':
      return largeShirts.push(reps.shirt_size);
    case 'XL':
      return largeShirts.push(reps.shirt_size);
...
ticketPriceTotal = runners.map(rep => {
  return rep.donation
}).reduce((total, amount) => {
  return ticketPriceTotal = total + amount;
}) 
