...
let people = runners.forEach(function(person){
...
let allCaps = runners.map((firstNameCaps) => {
    return firstNameCaps.first_name.toUpperCase();   
})
console.log(allCaps); 
...
let largeShirts = runners.filter((shirtSize) => {
...
let ticketPriceTotal = runners.reduce((totalDonation, donationAmount) => {
    return totalDonation + donationAmount.donation;
},0);
...
//set and the 5k fun run theme.  Try to create and then solve 3 unique problems 
//using one or many of the array methods listed above.


// Problem 1
let toString = runners.map((companyNameString) => {
    return companyNameString.company_name.toString();   
}) 
console.log(toString);

// Problem 2
let yahooEmails = runners.filter((emailType) => {
    return emailType.email = ".yahoo";
});
console.log(yahooEmails);

// Problem 3
let newRunner = runners.map((newPerson) => {
    return newPerson.first_name.toLowerCase();   
})
