...
let allCaps = [];
allCaps = runners.map(element => {return element.first_name.toUpperCase()})
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
largeShirts = runners.filter(element =>{return element.shirt_size==="L";})
...
// Problem 1
//retrieve a list of email addresses for an email blast 
let emailList = runners.map(element=> {return element.email})
console.log(emailList)

// Problem 2
//i want to use reduce to get the longest email address

let longestEmail = [];
longestEmail = runners.reduce((accumulator,currentValue)=>{
    if(currentValue.email.length > accumulator.length){
      return accumulator = currentValue.email;
    }else 
...
runners.forEach(element => {
  if (element.donation <= 50)
  {
    lowDonators.push({"FirstName":element.first_name, "LastName":element.last_name, "Email":element.email})
  }
})

console.log(lowDonators)