...
ticketPriceTotal = runners.reduce(function(accumulator, item) {
  ...
  console.log(`I am an individual Donation: ${item.donation}`);
...
let firstNames = [];
runners.forEach(function(items) {
  return firstNames.push(`${items.first_name}`);
});
...
runnersMediumSizeShirt = runners.filter(function(item) {
  return item.shirt_size === "M";
});

console.log(runnersMediumSizeShirt);

// Problem 3
let namesAllLower = [];

namesAllLower = runners.map(function(item) {
  return item.first_name.toLowerCase();
});

console.log(namesAllLower);
