...
runners.forEach(function (array) {
    let name = `${array.first_name} ${array.last_name}`
    fullName.push(name);
});
...
let allCaps = [];
runners.map(function (array) {
    let name = `${array.first_name}`
    allCaps.push(name.toUpperCase());
});
console.log(allCaps);
...
let largeShirts = [];
const largeSized = function(runner) {
