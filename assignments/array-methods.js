...
let ticketPriceTotal = runners.reduce(
  (ticketPriceTotal, runner) => (ticketPriceTotal += runner.donation),
  0,
);
...
const shirtSizes = runners.reduce((sizes, runner) => {
  !sizes[runner.shirt_size]
    ? (sizes[runner.shirt_size] = 1)
    : sizes[runner.shirt_size]++;

  return sizes;
}, {});
console.log(`We need the following shirt sizes: ${JSON.stringify(shirtSizes)}`);

// Problem 2
let minDonation = Number.MAX_SAFE_INTEGER;
let maxDonation = -1;
const minMax = runners.forEach(runner => {
  if (runner.donation > maxDonation) maxDonation = runner.donation;
  if (runner.donation < minDonation) minDonation = runner.donation;
});
/* prettier-ignore */
console.log(
  `The largest donation is $${maxDonation.toFixed(2)} and the smallest is $${minDonation.toFixed(2)}.`,
);

// Problem 3
const companies = [];
runners.forEach(runner => {
  companies.push({ [runner.company_name]: runner.donation });
});
