...
let largeShirts = [];
largeShirts = runners.filter(size => size.shirt_size === "L");
...
// Problem 1 - counting the numbers of each size shirt
let shirts = runners.map(shirts => shirts.shirt_size);
console.log(shirts);

let numbersOfShirts = shirts.reduce(function(allSizes, size) {
  if (size in allSizes) {
    allSizes[size]++;
  } else {
    allSizes[size] = 1;
...
console.log(numbersOfShirts);

// Problem 2 - lists which shirt size each id# should go with

let shirtID = runners.forEach(runner => console.log(`ID#${runner.id} goes on ${runner.first_name}\'s ${runner.shirt_size} shirt.`));

shirtID;

// Problem 3 - provide a total donation for a given company

let runnersFromCompany = [];
function companyRunnerList(company) {
  runnersFromCompany = runners.filter(comp => comp.company_name === company);
  return runnersFromCompany.reduce((acc, cur) => acc + cur.donation, 0);
}

console.log (companyRunnerList('Jaxbean'));
