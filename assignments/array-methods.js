...
let fullName = [];
runners.forEach(function (runArr) {
    fullName.push(runArr.first_name + " " + runArr.last_name)
})
...
largeShirts = runners.filter(function (filter) {
    return filter.shirt_size === "L"
})
...
ticketPriceTotal = runners.reduce(function (total, tally) {
...
// Problem 1 - Create  a list of last names for runner bibs and check-in
let lastName = [];

lastName = runners.map(function (runner) {
...
let runnerEmailList = [];
runners.forEach(function (runArr) {
    runnerEmailList.push(runArr.email + ", " + runArr.first_name + ", " + runArr.last_name)
})
