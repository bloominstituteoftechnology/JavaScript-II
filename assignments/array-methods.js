...
runners.forEach(function(item, index){
	fullName[index] = item["first_name"]+" "+item["last_name"];
});
...
let allCaps = [];
runners.map(function(item, index){
        allCaps[index] = item["first_name"].toUpperCase();
console.log(allCaps); 
...
console.log(donors.length);    //16


// Problem 2
//Covert all last names to lower case

let allLower = [];
runners.map(function(item, index){
        allLower[index] = item["last_name"].toLowerCase();
});
console.log(allLower);

// Problem 3 -number of people with shirt size small
let smallShirts = [];
smallShirts = runners.filter((item) =>{
        return item.shirt_size === 'S';
});
console.log(smallShirts.length); //11
