...
runners.map(function(big){
    allCaps.push(big.first_name.toUpperCase())
})
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce(function(acc,items){
    acc = acc + items.donation;
    return acc;
},0)
...
// Problem 1
// Selecting only people from one company
let peopleCompany = runners.filter(company => company.company_name === "Skinix")
console.log(peopleCompany)

// Problem 2
//Email List Print out
let emailList = []
runners.map(function(email){
