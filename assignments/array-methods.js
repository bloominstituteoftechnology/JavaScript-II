...
let allCaps = runners.map(function(item, index, array){
...
let largeShirts = runners.filter(function(item, index, array) {
...
let ticketPriceTotal = runners.reduce(function(sum, order) {
...
// Problem 1
let problemOne = runners.map(function(item, index, array) {
	return `${item.first_name} ${item.last_name} donated ${item.donation}`
});
console.log(problemOne);

// Problem 2
let problemTwo = runners.filter(function(item, index, array){
	return (item.donation > 100)
});
console.log(problemTwo);

// Problem 3
let problemThree = problemTwo.map(function(item, index, array) {
	return `${item.company_name} donated ${item.donation}`;
});

problemThree.forEach(function(item, index, array) {
	console.log(item);
});