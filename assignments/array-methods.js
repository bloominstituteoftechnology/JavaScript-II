...
newRunners.push(currentValue.first_name + " " + currentValue.last_name);
});
...
// Problem 1
//Need to send an e-mail to all participants, create a new array containing just the e-mail addresses.
runners.forEach(function(currentValue){
    console.log(currentValue.email);
});

// Problem 2
//Need to change all last names to Caps
let newData = runners.map(function(currentValue){
    return currentValue.last_name.toUpperCase();
});
console.log(newData);

//Need to find the information to people whose donations were over 200

let bigDonors = runners.filter(function(currentValue){
