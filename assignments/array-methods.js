...
let allCaps = runners.map(function(element) {return element['first_name'].toUpperCase()});
console.log(allCaps); 
...
ticketPriceTotal = runners.reduce((acc, next) => acc + next.donation, 0);
...
// Problem 1
let email = [];
runners.forEach(function(element){fullName.push(element.email)});
console.log(email);
...
// Problem 3
let companyName = [];
companyName = runners.map(function(element){
