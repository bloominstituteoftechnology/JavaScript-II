...
runners.forEach(runnerData => {
  fullName.push(runnerData.first_name + " " + runnerData.last_name);
});
...
let allCaps = [];
allCaps = runners.map(runnerData => {
  return runnerData.first_name.toUpperCase();
});
console.log(allCaps);
...
let largeShirts = [];
largeShirts = runners.filter(getLarge);

function getLarge(runnerData) {
  return runnerData.shirt_size == "L";
}
...
function donationTally(total, num) {
  console.log(num);
  return runnerData.donation.total + runnerData.donation.num;
}
