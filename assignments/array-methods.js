...
runners.map(function(item){
  allCaps.push(item.first_name.toLocaleUpperCase());
});
console.log(allCaps); 
...
let total = 0;
total = runners.reduce((theReducer, item) => {
...
/// Problem 1
//I need to know how many medium shirts we have
let medium = [];
runners.filter((item) => {
  if (item.shirt_size === "M"){
    medium.push(item.shirt_size);
  }
});
...
// Problem 3
// sort all the emails alphbeticaly and have it displayd in a list
let emails_sorted = [];
runners.forEach(function(item){
  emails_sorted.push(item.email);
});

emails_sorted = emails_sorted.sort();
console.log(emails_sorted)