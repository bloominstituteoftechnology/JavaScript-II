...
largeShirts = runners.filter( function(person) {
...
ticketPriceTotal = runners.reduce( function(donations, person) {
    return donations + person.donation;
}, 0);
...
// Problem 1
// Get email list using forEach 
let emailList = [];

runners.forEach( person => emailList.push(person.email) );
...
// Problem 2
// use reduce and get 1.5% times the donation to match.
let totalDonation = [];

totalDonation = runners.reduce( (donations, person) => donations + person.donation + (person.donation * .5), 0);

console.log(totalDonation); // 10564.5


// Problem 3
// Filter for employees of Skinix
let skinixEmployees = [];
