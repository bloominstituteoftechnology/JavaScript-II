...
let fullNames = [];
runners.forEach(function (racer, index){
  fullNames.push(racer.first_name + " " + racer.last_name)
})
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = runners.map(function(racer){
  return racer.first_name.toUpperCase();
});
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = runners.filter(function(racer){
...
// Problem 1 persons name and donation
let nameDonation = [];
runners.forEach(function (racer){
  nameDonation.push(racer.first_name + " $" + racer.donation);
});
console.log(nameDonation);

// Problem 2 get emails to send out a reminder
let emailList = runners.map(function(racer){
  return racer.email;
});
console.log(emailList); 

// Problem 3 name and company
let nameCo = [];
runners.forEach(function (racer){
  nameCo.push(racer.first_name + " " + racer.company_name);
});
console.log(nameCo);