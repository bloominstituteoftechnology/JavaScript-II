...
runners.forEach(runner => {
    const name = `${runner['first_name']} ${runner['last_name']}`
    fullName.push(name);
});
...
let donationsArr = runners.map(runner => {
    return runner['donation']
});
let ticketPriceTotal = donationsArr.reduce((total,current) =>{
...
// Problem 1
// Some of the racers have been very kind to donate more than $150, which is so cool! 
//The race officials want to send the SuperDonors an extra gift to thank them for their large donation. 
//Filter all racers that contributed $150 or more to the race.
...
// Problem 2
//The racing organization wants to know how many shirts of each size they need to order before the race. 
//Use filter to output each size and how many of each are needed. 

let xs = runners.filter(runner =>{
    return runner['shirt_size'] === "XS";
})

let small = runners.filter(runner => {
    return runner['shirt_size'] === "S";
})

let medium = runners.filter(runner =>{
    return runner['shirt_size'] === "M";
})

let large = runners.filter(runner =>{
    return runner['shirt-size'] === "L"
})

let xl = runners.filter(runner => {
    return runner['shirt_size'] === "XL"
})

let twoXL = runners.filter(runner => {
    return runner['shirt_size'] === "2XL"
})

let threeXL = runners.filter(runner => {
...
let sizes = [
    ...
    {"XL": xl.length - 1},
    {"2XL": twoXL.length - 1},
    {"3XL": threeXL.length - 1}
]

console.log(sizes);
// Problem 3
//The organization now wants an array of each racer and their contact info for permanent database storage. Use forEach
let contactInfo = [];
runners.forEach(runner => {
    const contact = `Name: ${runner['first_name']} ${runner['last_name']}, Email: ${runner['email']}`
