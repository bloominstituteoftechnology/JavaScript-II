...
const largeShirts = runners.filter(function(item) {
...
const ticketPriceTotal = runners.reduce(function(total, item) {
...
// Problem 2
// Check if first name includes the letter 'e'.  Return list of runners with 'e' in name.
const eRunners = runners.filter(function(item) {
    return (item.first_name.includes('e') || item.first_name.includes('E'));
});
console.log(eRunners);

// Problem 3
//  Multiply all donations by $100 to feel like we really raised some good money.
const bigMoney = runners.map(function(item) {
    return item.donation * 100;
});
const bigMoneyTotal = bigMoney.reduce(function(total, item) {
    return total + item
}, 0);
console.log(bigMoneyTotal);