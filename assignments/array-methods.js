...
let largeShirts = runners.filter(large => {
...
let ticketPriceTotal = runners.reduce((theReducer, donations) => {
  return (theReducer += donations.donation);
}, 0);
...
let newCompanies = runners.map(company => {
  return company.company_name;
});

console.log(newCompanies.sort());

// Problem 2
// Donations above $100
let newDonations = runners.filter(donations => {
...
// Problem 3
// Add up all the ID's
let idTotal = runners.reduce((theReducer, newId) => {
  return (theReducer += newId.id);
}, 0);
console.log(idTotal);
