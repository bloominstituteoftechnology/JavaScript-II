...
let fullName = runners.map((info) => {
    return `${info.first_name} ${info.last_name}`
});

// Oops didn't read instructions.  Could be done with forEach like this
// runners.forEach((info) => fullName.push(`${info.first_name} ${info.last_name})

console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = runners.map((info) => {
    info.first_name = info.first_name.toUpperCase()
    return info
});
console.log(allCaps); 
...
let largeShirts = runners.filter((info) => {
...
let ticketPriceTotal = runners.reduce((total, info) => {
    return total += info.donation
}, 0);
