...
let largeShirts = [];
largeShirts = runners.slice().filter(runner => runner.shirt_size === "L");
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal = runners.slice();
const total = ticketPriceTotal.reduce((accumulator,runner) => {
    accumulator += runner.donation;
    return accumulator;
}, 0);
console.log(total);
...
// Problem 1 | Big donators get a shoutout for advertising if donation is $200 or more |

const bigDonators = runners.slice();
const result = bigDonators.filter(runner => runner.donation >= 200);
result.forEach(runner => {
    console.log(`${runner.first_name} has donated ${runner.donation} and is running for ${runner.company_name} company!`);
});


// Problem 2 | Reaching a certain donation goal |

const donationTotal = runners.slice();

const donationResult = donationTotal.reduce((accumulator, runner) => {
    accumulator += runner.donation;
    return accumulator;
}, 0);

console.log(donationResult >= 1500);

// Problem 3 | The participants who received their shirts |

const shirtsInStock = runners.slice();
const notInStock = ['M', 'L'];
const soldToRunners = shirtsInStock.filter(runner => {
