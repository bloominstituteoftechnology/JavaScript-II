...
let ticketPriceTotal = runners.reduce(function(totalPrice, price) {
    return totalPrice + price.donation;
}, 0);
...
// Problem 1 top donors .filter()
let topDonors = runners.filter((amount) => {
    return amount.donation > 200
});

console.log(topDonors);

// Problem 2 send thank you cards .forEach()
let emails = [];
...
// Problem 3 Which company donated most .map() .reduce() .filter()
let companies = [];

runners.map(function(company) {
    companies.push(`${company.company_name} donated $${company.donation} dollars.`)
});
