...
let fullName = [];
fullName.push(runners.forEach(function(names){
...
let allCaps = [];
allCaps.push(runners.map(function(names){
console.log(`${names["first_name"].toUpperCase()}`)
}))
console.log(allCaps); 
...
let largeShirts = [];
largeShirts.push(runners.filter((name) => {
    if(name["shirt-size"] === "L"){
        return name;
    }
}))
...
ticketPriceTotal.push(runners.reduce((start,item) =>{
...
// Problem 1
//Manager Needs all the Emails
let Emails = []
Emails.push(runners.forEach(function(item){
    return item["email"];
}));
console.log(Emails)
// Problem 2
//The last contestant
let lastContestant = [];
lastContestant.push(runners.filter(function (item,index,array){
...
// Problem 3
// Who are the people that need XS shirts.
const XSShirts = [];
XSShirts.push(runners.filter(function(object){
    if(object.shirt_size === "XS"){
        console.log (`${object.first_name} ${object.last_name} needs a ${object.shirt_size}`);
    }
}))
