...
// Problem 1 .filter() .gov emails
let govEmails = [];
govEmails = runners.filter(runner => runner.email.includes('.gov'));
console.log(govEmails);

// Problem 2 .forEach reversed first names for the back of the shirts
let reversedFirstNames = [];
runners.forEach(runner => {
    let temp = [];
    for (let i = runner.first_name.length - 1; i >= 0; i--) {
        temp.push(runner.first_name[i]);
    }
    reversedFirstNames.push(temp.join('').toString());
});
console.log(reversedFirstNames);

// Problem 3 last names in the last half of alphabet
let lastNamesLastHalf = [];
lastNamesLastHalf = runners.filter(runner => runner.last_name[0] > 'N');
console.log(lastNamesLastHalf);