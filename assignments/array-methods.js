...
runners.forEach(function(combineNames) {
	fullName.push(combineNames.first_name + " " +combineNames.last_name);
});
...
// Problem 1: Donations > $100 

let donationsList = runners.filter((runner) => {
...
runners.forEach(function(namedJr) {
	nameJr.push(namedJr.first_name + ' ' + namedJr.last_name + ' ' + "Jr.");
});

// console.log(nameJr)

// Problem 3: the runners are like the writer bell hooks and prefer to be referred to in the lower case

let allLowerCaps = runners.map(function(runner){
     return runner.first_name.toLowerCase() + ' ' + runner.last_name.toLowerCase();
 });
