...
let fullName = [];
runners.forEach(function(currentObject) {
    fullName.push(currentObject.first_name+' '+currentObject.last_name);
});
...
let allCaps = [];
allCaps = runners.map(function(currentObject){
...
let largeShirts = [];
largeShirts = runners.filter(function (currentObject){
...
ticketPriceTotal = runners.reduce(function(accumulator,currentObject){
...
// Problem 1
//Find all the emails with a suffix '.com' and push them to an array
let emailDotCom=[];
emailDotCom=runners.filter(function(currentObject){
                            return currentObject.email.includes('.com');
});
console.log(emailDotCom);
// Problem 2
//Find all runners with under $100 of donation collected 
let littleDonation=[];
littleDonation=runners.filter(function(currentObject){
                                return currentObject.donation <100;
});
console.log(littleDonation);
// Problem 3
//Find all runners with shirt-size XL and above 
let tooBig=[];
tooBig = runners.filter(function(currentObject){
                        return currentObject.shirt_size.includes('XL');
});
console.log(tooBig);