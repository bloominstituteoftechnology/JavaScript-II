...
let fullName = [];
runners.forEach(
    let first = i.first_name;
let last = i.last_name;
let name = first + ' ' + last;
fullName.push(name);
...
let allCaps = [];
let caps = runners.map(i => i.first_name.toUpperCase();
allCaps.push(caps);
console.log(allCaps);
);
...
let largeShirts = [];
let large = runners.filter(
    i => i.shirt_size !=== "L";
largeShirts.push(large);
