...
//    fullName.push(arr[i].first_name + " " + arr[i].last_name)
    runners.forEach(function(id){fullName.push(id.first_name + " " + id.last_name)})
console.log(fullName)
// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = [];
    runners.forEach(function(i){allCaps.push(i.first_name)})
let realCaps = allCaps.map(i => i.toUpperCase())
...
let largeShirts = [];
//        runners.forEach(function(i){if(i.shirt_size === "L"){largeShirts.push(i.last_name)}})
...
let x = 0;
        runners.forEach(function(i){x = x+i.donation, ticketPriceTotal.splice(0, 1, x) })
