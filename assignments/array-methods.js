...
runners.filter(function(runner){
    if (runner.shirt_size === 'L') {
        largeShirts.push(`${runner.first_name} ${runner.last_name} ${runner.email}`);
    } 
});
...
ticketPriceTotal = runners.reduce(function(totalDonation, runnerID){
    return totalDonation + runnerID.donation;
}, 0);
console.log(ticketPriceTotal);

// I don't understand why I cannot push the return statement to the array. 


// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  
// I want you to think of potential problems you could solve given 
// the data set and the 5k fun run theme.  Try to create and then solve 
