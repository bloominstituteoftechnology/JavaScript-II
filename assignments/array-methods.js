...
// Problem 1: Adopt all of the runners into your family. Once the paperwork is complete, change all of their last names in the database to match yours.
let family = [];
function adopt(runner) {
    runner['last_name'] = "Heffernan";
    return runner;
}
family = runners.map(runner => adopt(runner));
console.log(family);

// Problem 2: Make the 5k fun run a 500k fun run. All of the runners have lost some weight by the end of the race and need shirts that are one size smaller than they originally ordered.
let sizes = ['XXS','XS','S','M','L','XL','2XL','3XL']
let smallerShirts = [];
function sizeDown(size) {
    let newSize = (sizes.indexOf(size) - 1);
    return sizes[newSize];
}
runners.forEach(runner => {
    runner['shirt_size'] = sizeDown(runner['shirt_size']);
    smallerShirts.push(runner);
});
console.log(smallerShirts);
