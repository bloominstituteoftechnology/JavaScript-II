...
let allCaps = [];
    ...
    runners.map(function(runners){
        return allCaps.push(runners.first_name);
    });
    ...
    //   console.log(allCaps.map(toUpper));
      console.log(allCaps.map(toUpper));
...
let total = runners.reduce(function(accumulator, runner){
...
// Problem 1
//Filter participants who donated more than $200.
// let bigSpenders = [];
let bigSpenders = runners.filter(function (runner) {
...
// Problem 2
// a local billionaire just announced that she will triple every runner's donation. Create a new array that will account for this.
let tripleDonate = runners.map(function(runner){
    return runner.donation * 3;

});
console.log(tripleDonate)


// Problem 3
//Using that new array, add up the new donation total.

    let newTotal = tripleDonate.reduce(function(total, runner){
        return total + runner;
