...
let allCaps = [];
runners.map(runner => {
    let powerCase = runner.first_name.toUpperCase();
    allCaps.push(powerCase);
});
console.log(allCaps); // could alter the entire array instead by storing the .map in it*
...
// Problem 1
// Produce a sorted list of all of the unique companies (members) in attendance.
const companies = [];
runners.forEach(runner => {
    let company = runner.company_name;
...
// Problem 2
// Now we want an object containing each respective company's cumulative donation. (Perhaps an array of objects would be more practical.)
const donations = {};
companies.forEach(company => {
    donations[company] = 0; // initializing company donations
});
runners.forEach(runner => {
    for (let company in donations) {
        if(company == runner.company_name) {
            donations[company] += runner.donation;
        }
    }
});
...
// Problem 3
// Give us an organized array of runners and their donations.
const runnersWithDonations = [];
runners.forEach(runner => {
    runnersWithDonations.push(`${runner.first_name} ${runner.last_name} donated $${runner.donation}.`);
});
console.log(runnersWithDonations);