...
let fullName = [];
let Names = runners.forEach(function(i) {
...
let allCaps = runners.map((rep) => {
    return rep.first_name.toUpperCase();
});
...
let ticketPriceTotal = runner.reduce((donationTotal, donationAmount) => {
...
// Problem 1
let repEmail = runners.map((email) => {
    return email.email;
});
