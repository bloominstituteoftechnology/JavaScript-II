...
let allCaps = [];
//runners.forEach((i) => allCaps.push(`${i.first_name.toUpperCase()}`));
allCaps = runners.map((item, index, array) => item.first_name.toUpperCase());
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
largeShirts = runners.filter((data, index, array) => data.shirt_size = "L")
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal = runners.reduce((reducer, elem) =>  reducer += elem.donation, 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 email List
const mailList = [];
runners.forEach((i) => mailList.push(i.email));
console.log(mailList);

// Problem 2 VIP and lowBaller List
const VIP = [];
const lowBaller = [];
runners.map((item, index, array) => {
    if (item.donation >= 200) {
        VIP.push(`${item.first_name} ${item.last_name}`);
    } else if(item.donation <= 100){
        lowBaller.push(`${item.first_name} ${item.last_name}`);
    }
});
console.log(VIP, lowBaller);
