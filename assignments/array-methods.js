...
runners.filter(function(runner){
    if(runner.shirt_size === 'L'){
        largeShirts.push(JSON.stringify(runner));
    }
});
...
const sum = runners.reduce((total, runner) => {
...
const sortedEmails = runners.map(function(runner){
    return runner.email;
}).sort();
...
// Find all runners from a given company
let companyRunners = [];
function runnerCo(company){
    runners.filter(function(runner){
        if(runner.company_name === company){
            companyRunners.push(JSON.stringify(runner));
        }
    });
...
// Problem 3
// Find out any runner's ID number
function findID(first, last){
    runners.forEach(function(runner){
        if(runner.first_name === first && runner.last_name === last){
