...
let ticketPriceTotal = runners.reduce(
  (total, state) => (total += state.donation),
  0
);
...
// Problem 1
// The director needs to contact everyone involved in the race. Create a new array with the person's first name: email address.
let runnersEmail = runners.map(
  runner => `${runner.first_name}: ${runner.email}`
);

console.log(runnersEmail);

// Problem 2
// The Director is giving a special prize for the companies that donated over $250. Please list the companies that donated over $250.
let donationPrize = runners.filter(runner => runner.donation > 250);

console.log(donationPrize);

// Problem 3
// The Director would like to rearrange the contact list by last name, first name. Please help them with that.
let reverseOrder = runners.map(
  runner => `${runner.last_name}, ${runner.first_name}`
);
