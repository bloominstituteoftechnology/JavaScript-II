...
let allCaps = [];
const capitalName = runners.map((item) => {
...
let largeShirts = [];
const result = runners.filter((item,array,index) => {
    if (item.shirt_size === "L") {
        largeShirts.push(item,array,index);
    }
});
...
const Donations = runners.reduce((total,amount,index, array) => {
