...
// Scroll to the bottom of the list to use some advanced array methods to help the event director gather some information
// from the businesses.
...
let ticketPriceTotal =
    runners.reduce(function(sum,amount){
       return sum+amount.donation;
...
// could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique
//  problems using one or many of the array methods listed above.

// Problem 1:How to keep a track of the enterprises who have participated and contributed
// more than 100 so that future relationship could be maintained?
//To solve this we will use filter() method:

let generousOrg = runners.filter(function(amount){
    return amount.donation>100;

});
console.log(generousOrg);


// Problem 2:How to keep record of company name and how much they have donated?
///To solve this we will use map() method

let recordKeeping= runners.map(function(item){
    return (item.company_name+" has donated "+item.donation);

});
console.log(recordKeeping);
// Problem 3:Since they capitalize the first-name so they also want to keep the last name capitalize.
//using map:
