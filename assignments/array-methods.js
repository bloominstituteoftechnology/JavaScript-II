// A local community center is holding a fundrasising 5k fun run and has invited 50 small businesses to make a small donation on their behalf for some much needed updates to their facilities.  Each business has assigned a representative to attend the event along with a small donation.
...
let ticketPriceTotal = runners.reduce((sum , company) => {
    return sum += company.donation;
}, 0);
...
// Problem 1: You want to give a shout out to the companies that donated more than $250.
let shoutOut = runners.filter((company) => {
    return (company["donation"]>250);
});
console.log(shoutOut);

// Problem 2: You want to send a mass email.
let massEmail = []
runners.forEach((runner) => {
    return (massEmail.push(runner["email"]));
});
console.log(massEmail);

// Problem 3: Since the companies donated through Pay Pal, Pay Pal decided to donate $0.05 for each dollar donated and you want to see how much Pay Pal will end up donating.
let PPDonation = runners.reduce((sum, company) => {
   return sum += (company.donation*.05)
},0);
console.log(PPDonation);
