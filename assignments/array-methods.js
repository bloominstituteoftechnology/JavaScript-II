...
ticketPriceTotal = runners.reduce((x,y) => x + y.donation,0);
...
// Problem 1
// Create an email distribution list for each of the runners
let emailDistro = [];
runners.forEach(item => emailDistro.push(item.email));
console.log(emailDistro);

// Problem 2
// Make a list of all runners that donated more than $250
let largeDonors = [];
largeDonors = runners.filter(item => {
    return item.donation > 250;
});
let largeDonorNames = [];
largeDonors.forEach(item => largeDonorNames.push(item.first_name+" "+item.last_name+": "+item.donation));
console.log(largeDonorNames);

// Problem 3
// Make a list of all runners that work for "Skinix"
let teamCompany = [];
const teamName = "Skinix";
teamCompany = runners.filter(item => {
    return item.company_name === teamName;
});
