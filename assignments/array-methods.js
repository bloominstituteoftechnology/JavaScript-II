...
function full_name(arr) {
  let fullName = [];
  arr.forEach(n => {
    fullName.push(`${n.first_name}, ${n.last_name}`);
  });
  return fullName;
}
...
function all_caps(arr) {
  let allCaps = arr.map(runner => {
...
function findShirtSize(arr) {
  let largeShirts = arr.filter(runner => {
...
function ticketDonations(arr) {
  let donationArray = arr.map(runner => {
    return runner.donation;
  });
  return donationArray.reduce((acc, currentValue) => {
