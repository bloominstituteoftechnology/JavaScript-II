...
runners.reduce(function(donations,item) {
  return (donations + item.donation);
}, 0)
console.log(ticketPriceTotal);

// const totalLandArea = 
// cityData.reduce(function(accumulator, item){
...
let contact = [];
runners.forEach(function(element){
  return (contact.push(`${element.email}`))
});
console.log(contact);
...
runners.map(function(element) {
  return (`${element.company_name} :${element.first_name} ${element.last_name}`);
});
