...
//****This solution will work but doesn't push to a new array*****//
// let firstNamesAllCaps = [];
// // const firstNamesAllCaps = runners.map(function(item) {
...
//****** The logic here is using the .filter method on the runners array, creating and anonymous function that takes an item as a parameter, using and if statement that when true or exists pushes the value of item into the empty array runnersLargeSizeShirts *******//

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let ticketPriceTotal = 0;
ticketPriceTotal = runners.reduce(function(accumulator, runner) {
  return accumulator + runner.donation;
}, 0);
console.log(ticketPriceTotal);

//***** Remember to include 0 at the end of the function for .reduce method *****/

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
// The event director wants to know who to treat the worst so he needs a list of all the scrubby runners.
let scrubRunners = [];
runners.filter(function(runner) {
  if (runner.donation < 50) {
    scrubRunners.push(runner);
  }
});
console.log(scrubRunners);

// Problem 2
// // The director wants a list of which runner is with what company
let companyNames = [];
runners.forEach(function(item) {
  companyNames.push(
    `${item.first_name} ${item.last_name} is with ${item.company_name}. `
  );
});
