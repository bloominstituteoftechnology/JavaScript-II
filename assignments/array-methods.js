...
//  })
 ...
 let allCaps =  runners.map(index => index.first_name.toUpperCase());
...
let ticketPriceTotal = runners.reduce((total, items) => total + items.donation, 0);
...
// Problem 1
let xLargeShirts = runners.filter(item => item.shirt_size === "2XL");

console.log(xLargeShirts);


// Problem 2
let lastOne =  runners.map(index => index.last_name.toLowerCase());
...
let Email = [];
runners.forEach(item => Email.push(`${item.email}`));
