...
let allCaps = runners.map(function(FIRST) {
	return FIRST.first_name.toUpperCase();
});
console.log(allCaps);
...
let largeShirts = runners.filter(function(sizeL) {
	return sizeL.shirt_size === 'L';
});
...
let ticketPriceTotal = runners.reduce(function(total, dono) {
	return (total += dono.donation);
}, 0);
...
runners.forEach(function(runner) {
	return thankYouNotes.sort().push(runner.company_name);
});
console.log(thankYouNotes);

// Problem 2
let emails = runners.map(function(runner) {
	return runner.email;
});
...
// Problem 3
let smallShirts = runners.filter(function(SizeS) {
	return SizeS.shirt_size === 'S';
});
