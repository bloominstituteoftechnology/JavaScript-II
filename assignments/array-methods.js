...
// Problem 1
// High donations
let highDonations = runners.filter((element) =>{
  return element.donation > 200;
});
console.log(highDonations);

// Problem 2
// Generate greetings for emails
let greetings = []
runners.forEach(function(element) {
  greetings.push(
  `${element.email}
  Thank you ${element.first_name} ${element.last_name} from ${element.company_name},`);
});
...
function sortName(arr, sortedArr) {
  arr.forEach(function(element) {
    sortedArr.push(element.last_name)
  })
  return sortedArr.sort();
}

console.log(sortName(runners, alphaLastName))