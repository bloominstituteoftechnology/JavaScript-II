...
// Problem 1
//  Return an array of the unique shirt sizes of the runners
let findPossibleSizes = function(runners)
{
    let copy = runners;
    let sizes = [];
    while(copy.length>0)
    {
        const currentShirtSize = copy[0].shirt_size;
        sizes.push(currentShirtSize);
        let temporary = [];
        copy.forEach(runner =>
        {
            if(runner.shirt_size!=currentShirtSize)
                     {
                        temporary.push(runner);
                     }
        })
        copy = temporary;
    }
...
console.log(findPossibleSizes(runners));

// Problem 2
// Return an array showing how many of each shirt size is needed.
let computeSizesArray = function(runners)
{
    let sizesArray = [];
    sizes = findPossibleSizes(runners);
    let copy = runners;
    while(copy.length>0)
    {
        const currentShirtSize = copy[0].shirt_size;
        ...
        sizesArray.push({'shirt_size':currentShirtSize, 'shirts_needed':accumulator});
        copy = temporaryArray;
    }
    return sizesArray;
}
console.log(computeSizesArray(runners));

// Problem 3
//Return an array of just e-mails for a universal notification list
let createEmailList = function(runners)
{
    return runners.map(runner => runner.email);
}
console.log(createEmailList(runners));
