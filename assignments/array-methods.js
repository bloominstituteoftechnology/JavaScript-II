...
let fullName = [];
 ...
 runners.forEach(i => {fullName.push({first_name: i.first_name, last_name: i.last_name})})
...
let allCaps = [];
allCaps = runners.map(i => i.first_name.toUpperCase())
console.log(allCaps); 
...
ticketPriceTotal = runners.reduce(function(totalPop, currentValue) {
    
    return totalPop + currentValue.donation;
...
// Problem 1
g = [];
g = runners.map(i => i.email)
console.log(g.sort())

// Problem 2
b = []
b =runners.map(i => i.company_name)
console.log(b)


large = runners.filter(i => i.shirt_size === "L" );
console.log(largeShirts.length +" " + "large");

medium = [];
medium = runners.filter(i => i.shirt_size === "M" );
console.log(medium.length + " " + "mediums");

small = [];
small = runners.filter(i => i.shirt_size === "S" );
console.log(small.length +" " + "smalls");