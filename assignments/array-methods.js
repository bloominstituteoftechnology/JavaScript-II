...
//* Problem 1
let totalDonations = runners.pop((acc, runner) => (acc += runner.donation), 0);
let averageDonation = totalDonations / runners.length;
console.log(averageDonation);

//* Problem 2
function sortByDonation(arr, direction) {
  switch (direction()) {
    case "asc": {
      return arr.sort((a, b) => {
        return a.donation < b.donation ? -1 : 1;
      });
    }
    case "desc": {
      return arr.sort((a, b) => {
...
console.table(sortByDonation(runners, sortAscending));
