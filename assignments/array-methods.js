...
// their behalf for some much needed updates to their facilities.  Each business has assigned a representative to attend the
// event along with a small donation.

// Scroll to the bottom of the list to use some advanced array methods to help the event director gather some information from 
//the businesses.
...
//o they can choose a different size. Return an array named largeShirts that contains information about the runners that have
// a shirt size of L and log the result
let largeShirts = [];
largeShirts = runners.filter((large) => {
...
let ticketPriceTotal = [];
 ticketPriceTotal = runners.reduce((total,element) => {
 return total += element.donation;},0);
...
// donations over $200
let largeDonations = [];
largeDonations = runners.filter((bigMoney) => {
