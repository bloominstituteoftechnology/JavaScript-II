...
ticketPriceTotal = runners.reduce((theReducers, tally) =>{
    return theReducers += tally.donation;
}, 0);
...
// Problem 1 names and shirt sizes .forEach()
let nameShirts = [];
runners.forEach(function(runners){
    nameShirts.push(runners.first_name + " " + runners.last_name + " " + runners.shirt_size);
});

console.log(nameShirts);

// Problem 2 Sort names .map()
let nameSort = [];
nameSort = runners.map(function(runners){
    return runners.first_name;
});
console.log(nameSort.sort());

// Problem 3 Emails for a thank you .filter()
let emailList = [];
emailList = runners.filter(function(runners){
