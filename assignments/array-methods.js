...
allCaps = runners.map(function(runners) {
    return runners.first_name.toLowerCase();
});
console.log(allCaps); 
...
// Problem 1
// Collect all participants emails into a single array.
...
// Problem 2
// Gather all company names into an array.

let companyName = [];
runners.forEach(function(runners) {
    companyName.push(`${runners['company_name']}`);
});
console.log(companyName);

// Problem 3
// Gather info on the top donators.

let highestDonations = [];
highestDonations = runners.filter(function(runners) {
    if(runners.donation > 290) {
        return runners.donation;
    }
});
console.log(JSON.stringify(highestDonations));
