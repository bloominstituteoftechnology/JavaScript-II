...
// console.log(runners.forEach(runners,null,fullNameCb));
...
// console.log(runners.map(allCapCb));
...
let shirtSet = runners.filter(result = () => {

   
    for (let i = 0; i < runners.length; i++){

         let size = runners[i].shirt_size;
         let name = runners[i].first_name;
        if (runners[i].shirt_size === 'L'){
            largeShirts.push(`${size} + ${name}`)
        }
    }
...
for (let i = 0; i < runners.length; i++){
//         let theBribe = runners[i].donation
...
let ticketPriceAvg = 0 ;
const sum = ticketPriceTotal.reduce((accumulator, value) => {

     ticketPriceAvg = accumulator + value ;

    return ticketPriceAvg ;
