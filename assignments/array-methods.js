...
let largeShirts = runners.filter((x) => {
...
let ticketPriceTotal = runners.reduce(function(x, y) {
    return x + y.donation;
}, 0);

let ticketPTotal = runners.reduce((x, y) => {
    return x + y.donation;
}, 0);

let ticketPriceTotalRoundTwo = runners.reduce((donationTotal, donationAmount) => {
    return donationTotal + donationAmount.donation;
}, 0);

console.log(ticketPriceTotalRoundTwo);

console.log(ticketPTotal);
...
// Problem 1
// Every runner who donated $200 or more gets a special prize. A list of everyone who donated $200 or greater needs to be made.

let largeDonors = runners.filter((x) => {
    if (x.donation >= 200) {
        return x.first_name;
    }
});
...
// Problem 2
// A list of all emails and names needs to be made so all participants may be emaild. Make a list of first names and email.

let emailListWithNames = [];
runners.forEach(function(x) {
    emailListWithNames.push(x.first_name + ", " + x.email);
});

console.log(emailListWithNames);

// Problem 3
// Wordtune is sponsering each of it's racers and is matching donations. Make a list of all donors who work at Wordtune as well as the amount of their donation.

let matchedDonations = [];

runners.filter((x) => {
    if (x.company_name === 'Wordtune') {
        return matchedDonations.push(x.first_name + " " + x.last_name + ", " + x.company_name + ", $" + x.donation);
    }
});

console.log(matchedDonations);
