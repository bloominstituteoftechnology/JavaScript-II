...
const firstNameUpperCase = (runner) => {return runner.first_name.toUpperCase()};
const allCaps = runners.map(firstNameUpperCase);
console.log(allCaps); 
...
let donationArray =[];
runners.forEach((runner) => {donationArray.push(runner.donation)});
const totalDonations = donationArray.reduce((accumulator, currentValue) => {return accumulator + currentValue}, 0);
