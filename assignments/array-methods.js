...
let ticketPriceTotal = runners.reduce((acc, next) => acc + next.donation, 0);
...
// Problem 1: Project an array of individuals who have donated greater than 200
// the projected array must have only 2 fields - a fullName field and a donation field
let greaterDonors = runners
  .filter(person => person.donation > 200)
  .map(person => ({
    fullName: `${person.first_name} ${person.last_name}`,
    donation: person.donation
  }));
console.log(greaterDonors);

// Problem 2: Find the person who has the highest donation using .reduce() method
let mostGenerousDonor = runners.reduce((acc, next) => {
  if (next.donation > acc.donation) return next;
  return acc;
});
console.log(mostGenerousDonor.first_name);

// Problem 3: Using the .reduce() method create an object that contains the number of shirts to order for each size
// The output object must contain { "M": <number of M>, "L": <number of L> ... }
let shirtOrders = runners.reduce(
  (acc, next) => ({
    ...acc,
    [next.shirt_size]: (acc[next.shirt_size] || 0) + 1
  }),
  {}
);
