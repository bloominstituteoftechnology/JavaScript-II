...
// Problem 1
// The event director needs to have all the runner's first name, last name and email
// to send out a thank you letter for participating on the fun run. Once you populated
// array nameAndEmail, log and use JSON.stringify() to make your list readable.
let nameAndEmail = [];
...
nameAndEmail.forEach(el => console.log(JSON.stringify(el)));

// Problem 2
// Change all the large shirt size to XL because there will be no available size for large shirts.
let largetoXL = [];
runners.forEach(runner => {
  if (runner.shirt_size === 'L') {
    runner.shirt_size = 'XL';
    largetoXL.push(runner);
  }
});

console.log(largetoXL);

// Problem 3
// Log all the runner's data that donated more than 100. They will be given a special
// prize.
let insanelyRichPeople = runners.filter(runner => runner.donation > 100);
console.log(insanelyRichPeople);