...
let allCaps = runners.map((item) => { return item.first_name.toUpperCase() });
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = runners.filter((item) => { return item.shirt_size == "L" });
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((accumulator, currentValue) => { 
    return accumulator + currentValue.donation}, 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce(). I want you to think of potential 
// problems you could solve given the data set and the 5k fun run theme. Try to solve 3 unique problems 
// using one or many of the array methods listed above.

// Problem 1 - Filter donations above 200
let largeDonations = runners.filter((item) => { return item.donation >= 200 });
console.log(largeDonations);
// Problem 2 - Sort alpha by last name and don't mutate the original array
let copyArray = [];
runners.forEach((item) => { copyArray.push(item) }); 
...
console.log(sortByLast(copyArray));
console.log(runners);
// Problem 3