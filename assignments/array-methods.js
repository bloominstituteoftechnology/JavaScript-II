...
let ticketPriceTotal = runners.reduce(function(arr, curVal) {
    return {donation: arr.donation + curVal.donation};
});
...
// Problem 1
// Add in the run times from an array that will be filled in after the 5k.  
const runTimes = [];
runners.forEach(function(element, index) {
    element.run_time = runTimes[index];
});

// Problem 2
// Donation 'Thank you' message.
runners.forEach(function(element){
    console.log(`Thank you ${element.first_name} ${element.last_name} from ${element.company_name} for you generous donation of \$${element.donation}.`)
});

// Problem 3
// A major coorperation caught wind of the fundraiser and would like to match 75% of each donation made by the small businesses.
let matchedDonations = runners.map(money => money.donation * .75).reduce((arr, curVal) => arr + curVal);
