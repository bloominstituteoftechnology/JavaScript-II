...
let NamewithCompany = [];
    runners.forEach(function(item){
        NamewithCompany.push(item.first_name + " " + item.last_name + " works at " + item.company_name);
}
);
console.log(NamewithCompany);


// Problem 2
let ticketPriceTotalAfterTax = [];

ticketPriceTotalAfterTax = runners.reduce((total, item) => total + item.donation*.6, 0);
console.log(ticketPriceTotalAfterTax);

// Problem 3
let goldMember = [];
goldMember = runners.filter((item) => {
