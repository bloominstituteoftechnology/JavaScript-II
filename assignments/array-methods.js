...
let allCaps = [];
allCaps = runners.map( function(results) {
    return results.first_name.toUpperCase();
  });
console.log(allCaps); 
...
let largeShirts = [];
largeShirts = runners.filter(results => {
    return results.shirt_size === "L";
  });
...
let sum = runners.reduce((acc, results) => {
    return acc + results.donation}, 0);
...
// Problem 1 - Email Contact List

let emailContactList = [];
runners.forEach(results=(item)=>{    
emailContactList.push(`Name: ${item.first_name} ${item.last_name} Email: ${item.email}`);
});
console.log(emailContactList);

// Problem 2 - Company Donor List

let companyDonorList = [];
runners.forEach(results=(item)=>{    
companyDonorList.push(`Company: ${item.company_name} Amount: $${item.donation}`);
});
console.log(companyDonorList);

// Problem 3 - Runner's Info Whose Company Donated Over $200

let largeDonors = [];
largeDonors = runners.filter(results => {
    return results.donation > 200;
  });
