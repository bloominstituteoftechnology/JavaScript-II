...
let ticketPriceTotal = runners.reduce((accumulator, cur) =>{ return accumulator+cur.donation; }, 0); 
...
//make an email list of high donors
let highDonor = runners.filter((x)=>{ if( x.donation > 200){return x.email}} );

// Problem 2

//creates a number for each shirt size 
let shirtTallies = {};
runners.forEach(element => {

    if (element.shirt_size in shirtTallies){
        shirtTallies[element.shirt_size]++;
    }
    else{
        shirtTallies[element.shirt_size] = 1;
    }
});
...
// Problem 3
//creates a unique company list
const companyList = runners.map(x => {
    return x.company_name;
});
const uniqueCompanyList = companyList.filter((v, i, a) => a.indexOf(v) === i); 
