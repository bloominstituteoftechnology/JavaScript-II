...
runners.forEach(function combineNames(item) {
...
let allCaps = [];
console.log(allCaps);
allCaps = runners.map(function toCaps(item) {
...
let largeShirts = [];
largeShirts = runners.filter(function largeShirtsOnly(item) {
...
ticketPriceTotal.push(runners.reduce(function donationAdder(accum, element) {
    return accum + element.donation;
}, 0));
