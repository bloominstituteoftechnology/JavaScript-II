...
const allCaps = runners.map(item => item.first_name.toUpperCase());
console.log(allCaps);
...
const ticketPriceTotal = runners.reduce(function (amount, money) {
    return amount + money.donation;
}, 0);
...
// Problem 1 Shirts only have space for 7 letters. Find people who need to use nickname.

const nickName = runners.filter(_names => _names.last_name.length > 7);
console.log(nickName);
console.log(nickName.length);

// Problem 2 Need to find people who donated more that $200 for special accolades

const highRollers = runners.filter(richer => richer.donation >= 200);
console.log(highRollers)

// Problem 3 need to add all emails to a communication list for everyone

const emailList = runners.map(emails => emails.email);
