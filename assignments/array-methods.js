...
largeShirtsNames = largeShirts.map(element => `${element.first_name} ${element.last_name}:${element.shirt_size}`);
...
// Dedicated function
{
    ...
    ticketPriceTotal = runners.map(element => element.donation);
...
let ticketPriceTotal = [];
ticketPriceTotal = runners.map(element => element.donation);
ticketPriceTotal = ticketPriceTotal.reduce( (first,second) => {return first + second});
console.log(ticketPriceTotal);
...
// Problem 1
// Put donations in packages. Filter, reduce donations, and classify donations by packages

// Problem 2
// Grab the names and filter by last name.

// Problem 3
// Count the number of shirt sizes