...
// Problem 1 
//The director wants to know the educational organizations attending to the even. Collect the companies that have emails ending with '.edu'

let edus = runners.filter(runner => runner.email.slice(-3) === 'edu')
console.log(edus)
// Problem 2
//The director wants to group bigger sized attendees in one table and sum up their donations for some weird reason. Filter the collection by checking if shirt size has 'X', and sum their donations.
let sizedTotalDonation = runners.filter(runner => runner.shirt_size.includes('X'))
    .reduce((total, runner) => {
        return total + runner.donation
    }, 0)
console.log(sizedTotalDonation)

// Problem 3
//The director wants to have a list of the names of the companies.
