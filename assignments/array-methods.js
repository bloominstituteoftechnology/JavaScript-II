...
runners.forEach(function (val){
    let bothNames = `${val.first_name} ${val.last_name}`;
...
// let allCaps = [];
const allCaps = runners.map(function(val){
...
// let largeShirts = [];
const largeShirts = runners.filter(function(val){
...
const ticketPriceTotal = runners.reduce(function(totalVal, val){
    return totalVal + val.donation;
}, 0);
...
// Problem 1
// Populating a list for email updates about the fun run
let emailList = [];
runners.forEach(function (val){
...
// Problem 2
// Grouping them into alphabetical flights for the fun run
const aThroughG = runners.filter(function(val){
    let splitUp = val.last_name.split('');
    return splitUp[0] =='A'| splitUp[0] =='B' | splitUp[0] =='C' | splitUp[0] =='D' | splitUp[0] =='E' | splitUp[0] =='F' | splitUp[0] =='G';
})
console.log(aThroughG);
//etc etc etc

// Problem 3
// Being able to search through the list to find all employees working for a given company
function companySearch(company){
    let employees = runners.filter(function(val){
        return val.company_name == company;
     });
    return employees;
};

console.log(companySearch('Kwimbee'));