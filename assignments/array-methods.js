...
//need to push the names to fullName but not sure how to do it.
...
runners.filter(runner => {if (runner.shirt_size == 'L') {largeShirts.push(`${ runner.first_name} ${ runner.last_name } `)}});

console.log('Shirts wont be avaiable for ' + largeShirts);
...
runners.reduce((acc, item) => {
    ...
    console.log('Donation Ammount:', acc)
    return acc 
  }, 0)
