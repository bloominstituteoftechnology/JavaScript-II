...
let fullName = [];
runners.forEach(function(first_Last) {
    return fullName.push(first_Last.first_name, first_Last.last_name);
});
...
let allCaps = [];
allCaps = runners.map((data, index, array)  => {
...
let largeShirts = [];
largeShirts = runners.filter((data, index, array) => {
...
ticketPriceTotal = runners.reduce((reducer, data, index, array) => {
    return reducer += data.donation;
}, 0);
...
// Problem 1 Let's Which company's are our biggest donners over 200 
let largestDonners = [];
largestDonners = runners.filter((data, index, array) => {
    return data.donation > 200;
});

console.log(largestDonners);

// Problem 2 Let's send an e-mail thanking our largest donors
let emailList = [];
runners.forEach(function(donorLarge) {
    return emailList.push(donorLarge.email);
});
console.log(emailList);



// Problem 3 We need of info for small shirt people

let numShirtSize = [];                                         
numShirtSize = runners.filter((data, index, array) => {
    return data.shirt_size === "S";
});

console.log(numShirtSize);