...
runners.forEach(function(currentValue) {
  return fullName.push(currentValue.first_name + " " + currentValue.last_name);
});
...
let ticketPriceTotal = runners.reduce(function(totalDonation, currentValue) {
  return totalDonation + currentValue.donation;
}, 0);
...
// Problem 1
//The event director wants to know who made the largest donation.
let largeDonation = runners.reduce(function(acc, currentValue) {
  if (acc.donation > currentValue.donation) {
    return acc;
  } else {
    return currentValue;
  }
...
// Problem 2
//The event manager is planning to order some T-shirts and wants to know how many small size T-shirt he needs to order.
let smallShirts = runners.filter(function(currentValue) {
  return currentValue.shirt_size === "S";
});
console.log(smallShirts.length);

// Problem 3
// The event director wants to greet all runners through emails, so he needs the list of emails of each runner.
let allEmails = runners.map(function(currentValue) {
