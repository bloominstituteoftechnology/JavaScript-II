...
let fullName = [];
runners.forEach(function(runnerDude) {
    console.log(runnerDude.first_name);
});

runners.forEach(function(runnerDude){
   fullName.push(`${runnerDude.first_name} ${runnerDude.last_name}`);
});
...
// let allCaps = runners.map(runnerDude => runnerDude.first_name).toUpperCase;
// allCaps;
let allCaps = runners.map(function(runnerDude){
    return runnerDude.first_name.toUpperCase();
});
...
let largeShirts = runners.filter(function(runnerDude){
    return runnerDude.shirt_size === 'L';
});
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce(function(donationTotal, runnerDude){
    return donationTotal + runnerDude.donation;
}, 0);
...
// Problem 1 email list for marketing campaign
let mailingList = runners.map(function(runnerDude){
    return runnerDude.email
});
console.log(mailingList)

// Problem 2 names of companies participating
let companiesParticpating = runners.map(function(runnerDude){
    return runnerDude.company_name
});
console.log(companiesParticpating)
