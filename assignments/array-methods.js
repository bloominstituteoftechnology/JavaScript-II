...
// The event director needs both the first and last names of each runner for
// their running bibs.  Combine both the first and last names into a new array
// called fullName. 
let fullName = [];

const combinedNames = runners.forEach((runner) => {
    let firstName = runner.first_name;
    let lastName = runner.last_name;
...
// uppercase because the director BECAME DRUNK WITH POWER. Convert each first name
// into all caps and log the result
let allCaps = [];

const nameToCaps = runners.map((runner) => {
    let capsName = runner.first_name.toUpperCase();
    allCaps.push(capsName);
})
console.log(allCaps); 
...
const largeRunners = runners.filter(runner => runner.shirt_size === 'L');
largeShirts.push(largeRunners);
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the
// donations into a ticketPriceTotal array and log the result
...
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to
// think of potential problems you could solve given the data set and the 5k fun run
// theme.  Try to create and then solve 3 unique problems using one or many of the
