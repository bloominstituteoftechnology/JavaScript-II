...
const tickets = runners.map(runner => runner.donation); 
let ticketPriceTotal = tickets.reduce((donationTotal,donation) => donationTotal + donation);
...
const LEGITTOTAL = LEGITRUNNERDONATIONS 
  .map(runner => runner.donation) 
  .reduce((total, donation) => total + donation);
