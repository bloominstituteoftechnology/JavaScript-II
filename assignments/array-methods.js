...
ticketPriceTotal = runners.reduce(function(total, amount) {
...
// Problem 1 -- We got too many Smalls!! Everyone who donated more than $200 and wears a size small gets an extra shirt!!!!!
let smallBodiesBigWallets = [];

smallBodiesBigWallets = runners.filter(function(runner){
    return runner.shirt_size === 'S' && runner.donation > 200;
});

console.log(smallBodiesBigWallets);
// Problem 2 -- I need everyone by lastname, firstname but in alphabetical order
let alphabeticalNames = [];

runners.forEach(function(runner){
    alphabeticalNames.push(`${runner.last_name}, ${runner.first_name}`);
});
alphabeticalNames.sort();

console.log(alphabeticalNames);

// Problem 3 -- the people who gave less than $25 goes in the array of shame (aka we'll contact them again for more money)

let reContact = [];

reContact = runners.filter(function(runner){
    return runner.donation < 25;
});
