...
// Problem 1
//The organizers want to send a thank you email to runners so we use .forEach() to populate the email addresses of the runners.

let allEmail = []
 runners.forEach(function(getContact){
    allEmail.push(getContact.email);
});
 console.log(allEmail)

// Problem 2
/* There has been a delay in printing shirts Medium and below due to technical issues. We need to get the information of all runners with shirt sizes Medium, Small, and Extra Small, to notify them
*/

let mediumBelow = runners.filter(function(lessThanMed){
    return lessThanMed.shirt_size === "M" || lessThanMed.shirt_size === "S" || lessThanMed.shirt_size === "XS";
});
console.log(mediumBelow);
