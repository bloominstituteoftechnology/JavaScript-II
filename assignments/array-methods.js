...
let largePeople = runners.filter(large => {
    return large.shirt_size === "L";
})

largeShirts.push(largePeople);
...
let donations = runners.map(money => {
    return money.donation;
})

let total = donations.reduce((acc, curr) => {
    return acc+ curr;
})
...
let skinex = runners.filter(companyName => {
    return companyName.company_name === "Skinix"})
