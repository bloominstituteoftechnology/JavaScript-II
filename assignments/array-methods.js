// A local community center is holding a fund rasising 5k fun run and has invited 50 small businesses to make
// a small donation on their behalf for some much needed updates to their facilities.  Each business has assigned
...
allCaps.push(runners.map(function(runner) {
    return runner.first_name.toLocaleUpperCase();
}));
...
let largeShirts = [];
largeShirts.push(runners.filter(function(runner) {
...
ticketPriceTotal.push(runners.reduce(function(totalDonation, runner) {
    return totalDonation + runner.donation;
}), 0);
...
// you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems
...
// Problem 1
// For whatever reason, the director needs all of the companies that are participating in alphabetical order. Get
// to it, slave.
let participatingCompanies = [];
runners.forEach(function(runner) {
  participatingCompanies.push(runner.company_name);
});

console.log(participatingCompanies.sort());

// Problem 2
// You've made the rookie error: you made it known you're useful. Now, the director is only coming to you for all of
// their "code monkey" needs. Give them all of the runners names, first and last, along with their email.
let runnerEmail = [];
runnerEmail.push(runners.map(function(runner) {
...
// Problem 3
// Turns out the director is some kinda pervert. They wants the information of all the runners with small shirt sizes.
// Your mission, should you accept, is to run interference by offering them the information on those with 3XL sized
// shirts while command conducts a thorough and very illegal background check.

let runnerEmailModified = [];
runnerEmailModified.push(runners.filter(function(runner) {
    return runner.shirt_size = "3XL";
  }
));

console.log(runnerEmailModified);