...
let fullNames = [];
runners.forEach(runner => fullNames.push(`${runner.first_name} ${runners.last_name}`));
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = [];
runners.map(runner => firstNamesAllCaps.push(runner.first_name.toUpperCase()));
...
// let firstNames = [];
// console.log(runners.filter(runner => runner.first_name.startsWith('M')));
// // Problem 2
