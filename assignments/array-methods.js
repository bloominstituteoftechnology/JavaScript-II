...
let fullName = [];
const forEachCB = (item) => {
	let runnerFullName = `${item.first_name.toString()} ${item.last_name.toString()}`;
	fullName.push(runnerFullName);
}

runners.forEach(forEachCB);
...
let allCaps = [];
const firstNameCapsCB = (item) => {
	let runnersName = item.first_name.toUpperCase().toString();
	return runnersName;
}
allCaps = runners.map(firstNameCapsCB);
