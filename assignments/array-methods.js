...
ticketPriceTotal = runners.reduce((totalDonations, runners) => totalDonations + runners.donation, 0);
...
// Problem 1
// the date for the run has changed! We need to grab the first name and email address for each runner and return an array with it so we can email everyone about the new date.
const emailList = runners.map(runner => `${runner.first_name}'s email is ${runner.email}`);
console.log(emailList);

// Problem 2
// A few of our runners have been exceptionally lucky with their donations recieved. For this, we would like to give those whos donations were over $250 a special thank you patch on their shirts. return an array on all runners whose donations were over $250
const highDonations = runners.filter( runner => runner.donation > 250);
console.log(highDonations);

// Problem 3
// A generous donor has decided to add $20 to every runners donation total. We need you to update the donations totals inside the runners array and return the resulting donations into the new total
const newDonationsTotal = runners.reduce((totalDonations, runners) => totalDonations + runners.donation + 20, 0);

console.log(newDonationsTotal);
