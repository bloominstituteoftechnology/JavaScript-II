...
let fullName = [];
const name = runners.forEach(function(runners){
...
let allCaps = [];
const caps = runners.map(function(runners){
...
let largeShirts = [];
const shirtsize = runners.filter(function(runners){
  if (runners.shirt_size === "L"){
    largeShirts.push(runners.first_name)
  };
})
...
let ticketPriceTotal = runners.reduce(function(totalpeople, currentValue){
    return totalpeople += currentValue.donation;
  },0);
