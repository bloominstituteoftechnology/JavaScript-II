...
allCaps = fullName.map(function(name){
  return name.substr(0, name.indexOf(" ")).toUpperCase() + " " + name.substr(name.indexOf(" "));
});
console.log(allCaps);
...
let largeShirts = [];
largeShirts = runners.filter(newArray => newArray.shirt_size == "L")
...
var donationList = [];
runners.forEach(function(ele){
  donationList.push(ele.donation);
});
ticketPriceTotal = donationList.reduce((acc, val) => acc + val)
