...
runners.forEach(F_L => {
  return fullName.push(`${F_L.first_name}  ${F_L.last_name}`);
});
...
const upperNames = runners.map(runner => {
  return runner.first_name.toUpperCase();
});
allCaps.push(upperNames);
...
const largeRunner = runners.filter(runner => {
  return runner.shirt_size === 'L';
});

largeShirts.push(largeRunner);
...
const donations = runners.reduce((total, amount) => {
  return (total += amount.donation);
}, 0);

ticketPriceTotal.push(donations);
...
// Problem 1
// The event embastor needs all of the last names upercase and in a list.
CaptialLastName = [];
runners.forEach(runner => {
  return CaptialLastName.push(runner.last_name.toUpperCase());
});

console.log(CaptialLastName);

// Problem 2
// The event embastor needs all of the runners information that donated less than 40$ so they know who not to invite to the after party.

cheepSkates = [];

const brokeRunners = runners.filter(runner => {
  return runner.donation > 39;
});

cheepSkates.push(brokeRunners);

console.log(cheepSkates);

// Problem 3
//The event host needs to know how many runners are in the race.

let totalRunnners = [];

const totalNumber = runners.reduce((total, runner) => {
  return (total += runner.id / 25.5);
}, 0);

totalRunnners.push(totalNumber);

console.log(totalRunnners);
