...
largeShirts = runners.filter((runners, index, array) =>{
...
ticketPriceTotal = runners.reduce((theReducer, item, index, array) =>{
...
// Problem 1 tally up all the dontations from the Skinix company employees
let skinixEmployees = runners.filter((runners, index, array) =>{
    return runners.company_name === "Skinix"; 
  })
  
let skinixDonationCount = skinixEmployees.reduce((theReducer, item, index, array) =>{   
    return theReducer += item.donation;
  },0);
  
  
  console.log(skinixDonationCount)
