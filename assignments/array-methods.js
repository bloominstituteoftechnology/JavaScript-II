...
let ticketPriceTotal = runners.reduce(function(total, item){
...
// Problem 1
let companies = runners.map(function(item) {
  return item.company_name;
});

function thankYou(arr) {
  return `Thank you ${arr} for participating.`
}

console.log(thankYou(companies));

// Problem 2
let totalDonation = runners.reduce(function(total, item) {
  return total += item.donation;
},0)

function matchDonation(arr) {
  return `Thank you Anonymus for matching the total donation. We raised ${totalDonation*2}.`;
}


console.log(matchDonation(totalDonation));



// Problem 3
let badDonater = runners.filter(function(item){
  return item.donation < 20;
})

let email = badDonater.map(function(item){
  return `${item.email}: ${item.first_name} you raised less than $20. Try harder next year.`
})
