...
// let fullName = [];
// fullName = runners.map(function(name) {
...
let allCaps = [];
allCaps = runners.map(function(caps) {
...
const emails = runners.map((runner) => {
    return runner.email;
});
console.log(emails);

// Problem 2: Average donation
const entries = runners.length;
let averageDonation = runners.reduce((acc,currentValue) => {
    return acc += currentValue.donation;
},0);
console.log(averageDonation/entries);

// Problem 3: Alphabetize by last name
let sorted = [];
sorted = runners.map((runner) => {
