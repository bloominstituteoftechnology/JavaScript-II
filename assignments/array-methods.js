...
ticketPriceTotal = runners.reduce(function(total, item){
...
// Problem 1: Disqualify everyone with a .net email address

let qualifiedList = [];

qualifiedList = runners.filter(function(item){
    return !item.email.endsWith('.net');
})

console.log(qualifiedList);


// Problem 2: Add an extra $1000 to everyone who donated less than $100
...
runners.forEach(function(item){
    ...
    if(item.donation < 100){
        newItem.donation = newItem.donation + 1000;
        betterDonations.push(newItem);
    } else {
        betterDonations.push(item);
    }
})

console.log(betterDonations);

// Problem 3: Capitalize the first names of everyone who has a 3XL shirt size

let capitalizePlusSize = [];

capitalizePlusSize = runners.map(function(item){
    if (item.shirt_size === '3XL') {
        item.first_name = item.first_name.toUpperCase(0);
        return item;
    }
    return item;
})

console.log(capitalizePlusSize);