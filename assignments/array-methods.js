...
let fullName = [];
const combine = (item, index, array) => {
    let name = `${array[index].first_name} ${array[index].last_name}`
    fullName.push(name);
}
runners.forEach(combine);
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = [];
const caps = (item, index, array) => {
    allCaps.push(array[index].first_name.toUpperCase());
}
runners.map(caps);
console.log(allCaps);
...
let largeShirts = [];
const shirts = (item, index, array) => {
    if (array[index].shirt_size === "L") {
        largeShirts.push(array[index]);
    }
}
...
ticketPriceTotal.push(runners.reduce(sum, runners[0].donation))
...
// Problem 1
//Get emails and names of runners who donated more than 200
let prefferedRunners = [];
const greatDonation = (item, index, array) => {
    if (array[index].donation >= 200) {
        prefferedRunners.push(`${array[index].first_name} ${array[index].last_name} donated ${array[index].donation} their email is ${array[index].email}`)
    }
}
runners.filter(greatDonation)
console.log(prefferedRunners)


// Problem 2
//Gather info needed for ID's including first and last name, company, and ID
let runnerinfo = [];
const runnerTag = (item, index, array) => {
    runnerinfo.push(`${array[index].first_name} ${array[index].last_name} ${array[index].company_name} ${array[index].id}`)
}

runners.map(runnerTag);
...
const countShirts = (item, index, array) => {

    if (array[index].shirt_size === "XS") {
        shirtsInventory.XS++;
    } else if (array[index].shirt_size === "S") {
        shirtsInventory.S++;
    } else if (array[index].shirt_size === "M") {
        shirtsInventory.M++;
    } else if (array[index].shirt_size === "L") {
        shirtsInventory.L++;
    } else if (array[index].shirt_size === "XL") {
        shirtsInventory.XL++;
    } else if (array[index].shirt_size === "2XL") {
        shirtsInventory.XXL++;
    } else {
        shirtsInventory.XXXL++;
    }
}
runners.forEach(countShirts);
console.log(shirtsInventory);