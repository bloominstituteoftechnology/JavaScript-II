...
ticketPriceTotal = runners.reduce(
  (totalDonations, runner) => totalDonations + runner.donation,
  0
);
...
// Problem 1
// The event's website has been hacked and personal information of some of the runners has been compromised!
// The hackers only managed to get the info of runners if they had a .edu email address. Using .filter, create an array that contains all the compromised runners' info so you can tell them "Our bad, bro".
let hackedEmails = [];
hackedEmails = runners.filter(
  runner => runner.email.slice(runner.email.length - 4) === ".edu"
);
console.log(hackedEmails);

// Problem 2
// Due to a decrease in faith in your charity from the hacking incident, donors are still agreeing to run for awareness but have decided to take back their donations!
// The business owners all stormed your office and each demanded $100 back, regardless of how much they donated. Your new intern Jimmy is an idiot and complied without calling the director.
// Use .map() to lower everybody's contributions by $100. If this would lower their donation below 0, store those runners in a new array dirtyThieves
let dirtyThieves = [];
runners.map(runner => (runner.donation -= 100));
dirtyThieves = runners.filter(runner => runner.donation < 0);
console.log(dirtyThieves);

// Problem 3
// Due to Jimmy's mistake in giving owners their money back, your charity has now fallen apart around you, you are being investigated for tax evasion, your spouse has left you, and your dog is no longer respecting your authority.
// The people who wronged you must pay, and Jimmy has connections with the mob (allegedly) and wants his family to 'help'.
// Use .map() to add a property "kneecaps" for each runner in the original array, and set it to "intact" if their donation is still a positive amount, or "broken" if it is less than 0.
runners.map(
  runner =>
    runner.donation < 0
      ? (runner.kneecaps = "broken")
      : (runner.kneecaps = "intact")
);
console.log(runners);
