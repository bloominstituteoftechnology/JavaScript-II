...
runners.map(function(upCase){
  allCaps.push(upCase.first_name.toUpperCase());
}
 )
console.log(allCaps);
...
const sumAll = (accumulator, currentValue) => accumulator + currentValue;
 tempArr = [];
runners.map(function(totals){
 tempArr.push(totals.donation);
})
 ticketPriceTotal = tempArr.reduce(sumAll);
