...
//     first = runners[i].first_name;
//     last = runners[i].last_name;
...
let allCaps = [];
runners.map(function(firstNameCap) {
    return allCaps.push(firstNameCap.first_name.toUpperCase())
})
console.log(allCaps); 
...
let largeShirts = [];
runners.filter(function(noshirtSize) {
    if (noshirtSize.shirt_size === "L") {
        return largeShirts.push(noshirtSize.first_name + ' ' + noshirtSize.last_name)
    }
})
