...
let ticketPriceTotal = runners.reduce((sum, runner) => {
...
// Problem 1 - making "teams" from the same company
let teamSkinte = runners.filter((runner) => runner.company_name === "Skinte");
console.log(teamSkinte);

// Problem 2 - find the top donator
let topDonor = runners.reduce((top, runner) => {
    if(top < runner.donation){
...
// Problem 3 - get all email address to send out reminder
let emailAddress = runners.map((runner) => runner.email);
