...
let allCaps = [];
allCaps = runners.map(function(obj){
...
let largeShirts = [];
largeShirts = runners.filter(function(obj){
  return obj.shirt_size === "L";
});
console.log(`\n*** Challenge 3 ***`);
for (let i in largeShirts) {
  console.log(`Name: ${largeShirts[i].first_name} ${largeShirts[i].last_name}`);
  console.log(`Shirt Size: ${largeShirts[i].shirt_size}\n`);
}

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal = runners.map(function(obj){
  return obj.donation;
});

ticketPriceTotal = ticketPriceTotal.reduce(function(acc, val){
  return acc + val;
});
console.log(`\n*** Challenge 4 ***`);
