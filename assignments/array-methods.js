...
allCaps = runners.map(caps => {
  let firstCaps = {};
  firstCaps[caps.first_name.toUpperCase()] = caps.last_name;
  return firstCaps;
});
console.log(allCaps);
...
ticketPriceTotal = runners.reduce((a, b) => ({ donation: a.donation + b.donation }));
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 - Filter top donators to personally thank them for their contribution (any sum over)
let mvp = [];
runners.forEach(element => {
  if (element.donation >= 100) {
    mvp.push(element.donation);
  }
});
console.log(mvp);

// Problem 2 - Filters out an email list with first name last name and email address
let eList = [];
runners.forEach(element => {
  eList.push(`First Name: ${element.first_name}, Last Name: ${element.last_name}, Email: ${element.email}`);
});
console.log(eList);

// Problem 3 - Filters out full name with their shirt size for ordering purposes
let order = [];
runners.forEach(element => {
  order.push(`Name: ${element.first_name} ${element.last_name}, Shirt Size: ${element.shirt_size}`);
});
console.log(order);