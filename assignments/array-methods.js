...
// Problem 1: Find the total number of donations collected by company_name
const companies = []
runners.forEach(runner => {
  if(!companies.includes(runner.company_name))
    companies.push({
      company_name: runner.company_name,
      donation: runners.filter(runner2 => runner2.company_name === runner.company_name).reduce((acc, curr) => acc + curr.donation, 0)
    })
})

console.log(companies);

// Problem 2: Find highest donatoin runner
let highestRunner = runners[0]
runners.forEach(runner => {
  if (runner.donation > highestRunner.donation){
    highestRunner = runner
  }
})

console.log(highestRunner);

// Problem 3 find all email domains
const domains = runners.map(runner =>runner.email.split('@')[1]);
