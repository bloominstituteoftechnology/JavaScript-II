...
let ticketPriceTotal = runners.map(function(element) {
									return element.donation; }) 
							  .reduce(function(total, donation) { 
...
// Problem 1 :----  To find out highest donation runner
function maxDonationRunner(arr) {
	let max = 0;
	let runnerName = "";
	runners.forEach(function(element) {
		if(max < element.donation) {
			max = element.donation;
			runnerName = element.first_name + " " + element.last_name; 
		}
	});
return runnerName;
}

console.log(maxDonationRunner(runners)); // invoking funtion maxDonationRunner to find out maximum donation runner from runners.
