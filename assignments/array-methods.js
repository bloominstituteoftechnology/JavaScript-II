...
let ticketPriceTotal = runners.reduce((donation, runner) => { return donation += runner.donation}, 0);
...
// The event director decided that 50 people is too many to be running at the same time. He decides to split them into two groups: one for id's less than or equal to 25 and one for those greater than 25.
// Divide the runners into these two groups.

let idUnderTwentyFive = runners.filter((runner) => runner.id <= 25 )

let idOverTwentyFive = runners.filter((runner) => runner.id > 25 )

//console.log(idUnderTwentyFive)
//console.log(idOverTwentyFive)

// Problem 2

// The event director wants to send thank you packages to each of the companies. Get each of the companies into an array with no duplicates.
let companies = []

runners.forEach((runner) => companies.push(runner.company_name));
...
var noDuplicateCompanies = companies.filter(unique);

console.log(noDuplicateCompanies)

// Problem 3

// The event director wants all the last names to be uppercase, too. Convert the last names to uppercase. 

let allCapsLast = runners.map((runner) => runner.last_name.toUpperCase());

console.log(allCapsLast)