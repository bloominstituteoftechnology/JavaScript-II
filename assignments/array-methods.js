...
let fullName = [];
let participantNames = runners.forEach(function(i){
...
// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first
// names converted to uppercase because the director BECAME DRUNK WITH POWER. 
//Convert each first name into all caps and log the result

let firstnameCaps = runners.map((individuals)=> {
    return individuals.first_name.toUpperCase();
});
...
//Return an array named largeShirts that contains information about the runners that 
//have a shirt size of L and log the result

let largeShirts = runners.filter(function(notEnough){
    return notEnough.shirt_size === "L";
});
...
let ticketPriceTotal = runners.reduce(function(donationTotal, donationAmount){
...
// I want you to think of potential problems you could solve given the data set and the 5k fun run
// theme.  Try to create and then solve 3 unique problems using one or many of the array methods 
//listed above.
...
let compName = [];
let companies = runners.forEach(function(i){
    compName.push(i.company_name);
    compName.sort();
});
...
// Problem 2
// I need all 3XLs to report immediatley -- so that I can give them a diff. size. (it's 
// going to be a tight squeeze)

let threexlShirts = runners.filter(function(notEnough){
    return notEnough.shirt_size === "3XL";
});
console.log(threexlShirts);


// Problem 3
 //I need all last names capitalized

 let lastnameCaps = runners.map((individuals)=> {
    
    return individuals.last_name.toUpperCase();
