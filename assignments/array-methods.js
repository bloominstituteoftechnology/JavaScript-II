...
runners.forEach(runner => {
  return runnersContact.push(`Name: ${runner.first_name} ${runner.last_name} Email: ${runner.email}`);
})

console.log(runnersContact);

// Problem 2
let runnerDonation = runners.filter(runner => {
...
// Problem 3
let companyNameAllCaps = [];

runners.map((runner, index) => {
  companyNameAllCaps[index] = runner.company_name.toUpperCase();
});
console.log(companyNameAllCaps);