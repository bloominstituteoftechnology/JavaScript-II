...
let ticketPriceTotal = runners.reduce((total, runner) => {
    return parseInt(total) + parseInt(runner.donation)
}, 0);
...
/////////////////////////////////////////////////////////////////////////////
// Problem 1
// Email each runner with API

async function emailRunners() {
    // Loops through each runner and emails them
    for (let runner of runners) {
        let res = await email(runner.email);
        if (res.success) console.log("Email sent successfully!", res.email);
    }
}
...
function email(email) {
    // Calls an imitation email API and returns a promise
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            // Imitate email API w/ timeout
...
/////////////////////////////////////////////////////////////////////////////
// Problem 2
// Force each runner to pay double the donation

let doubleDonations = runners.map(runner => {return {...runner, donation: runner.donation * 2}});

console.log(doubleDonations);

/////////////////////////////////////////////////////////////////////////////
// Problem 3
// Get the quantity of each shirt size so we know how many of each to order

const shirtSizes = runners.reduce( (sizes, runner) => {
    sizes[runner.shirt_size] = (sizes[runner.shirt_size] || 0) + 1 ;
    return sizes;
} , {});
