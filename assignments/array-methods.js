...
runners.map(item) => {
	allCaps.push('${first_name}'.toUpperCase());
}
console.log(allCaps); 
...
largeShirts.push(runners.filter(element => element.shirt_size === "L"));
...
ticketPriceTotal = runners.reduce((total, currentValue) => {
	return total += currentValue.donation; }, 0);
...
// Problem 1: The event director needs the email address of each runner.  Log the email address of each runner into a new array called emailAddress. 
let emailAddress = [];

runners.forEach (item) => {
	emailAddress.push ('${emai}');
}

console.log(emailAddress);

// Problem 2:  Get a list of runners that are with the company WordTune. Return an array named wordTune that contains information about the runners that are with WordTune and log the result
let wordTune = [];

wordTune.push(runners.filter(element => element.company_name === "Wordtune"));

console.log(wordTune);

// Problem 3: // The event director needs to have all the runner's first names converted to lowercase. Convert each first name into lowercase and log the result

let noCaps = [];

runners.map(item) => {
	noCaps.push('${first_name}'.toLocaleLowerCase());
}
console.log(noCaps); 