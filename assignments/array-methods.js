...
let ticketPriceTotal = runners.reduce((acc, runner, index) => {
    acc += runner.donation;
    return acc;
}, 0)
...
// Problem 1
// Runners 24 and 8 are banned from the race. We need a list with banned runners.
let bannedRunners = runners.filter(runner => runner.id === 24 || runner.id ===8);
console.log(bannedRunners);
// Problem 2
// Runners 32 wanted to contribute more to donations. There information needs to be updated. 
runners.filter(runner => {
    if(runner.id === 32){
    runner.donation += 100;
    console.log(runner.donation)
    }
    })
// Problem 3