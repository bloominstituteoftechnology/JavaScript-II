...
let allCaps = runners.map(function(upper) {
    return upper.first_name.toUpperCase();
})
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can 
// choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log
//  the result.
...
const ticketPriceTotal = runners.reduce(function(donate,money) {
    return donate + money.donation;
}, 0);
...
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the 
// data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 - Email address and names of all the runners 
let Email = [];
runners.forEach(function(runner){
    Email.push(`${runner.email} ${runner.first_name}`);
})
console.log(JSON.stringify(Email));

// Problem 2 - Everyone who donated over 100$ gets a special gift. Filter the runners who donated over $100

let Prize = runners.filter(function(winners){
    return winners.donation > 100;
})
console.log(Prize);

// Problem 3 - Information of all the runners who work for Skinix 
let Company = runners.filter(function(skin){
    return skin.company_name === "Skinix";
})
console.log(Company);