...
let ticketPriceTotal = runners.reduce((acc, x) => {return acc + x["donation"]}, 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 E-mail list:

const emailList = runners.map(x => x.email);
console.log(emailList);

// Problem 2 Large donations list

const largeDonors = runners.filter(x => x.donation >= 250);
console.log(largeDonors);

// Problem 3 Average donations

const average = runners.reduce((acc, x) => {return acc + x["donation"]}, 0) / runners.length;
console.log(average);