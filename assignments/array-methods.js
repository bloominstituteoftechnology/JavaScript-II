...
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
 let allCaps =runners.map(function(runners) {
...
let largeShirts = runners.filter(function (runners) {
  let sizeLarge = (runners.shirt_size === 'L');
  return sizeLarge;
});
...
let ticketPriceTotal = runners.reduce(function(accum, item) {
...
// Problem 1
//The director wants to give nicer medals to the runners that donated more then $100 using .filter() make a list with only the runners that donated more than $100
let bigDonars = runners.filter(function(runners) {
  let overOneHundred = (runners.donation > 100);
  return overOneHundred;
})

console.log(bigDonars)
// Problem 2 For the event all the runners get customized banners that say their first and last name and that they completed the 5K do this using .forEach()
let runnerCustomizedBanner = [];
runners.forEach(function(runners) {
  runnerCustomizedBanner.push(`My name is ${runners.first_name} ${runners.last_name} and I completed the 5k in one piece!!`)
})

console.log(runnerCustomizedBanner)
// Problem 3
let donationAverage = runners.reduce(function(accum, item) {
