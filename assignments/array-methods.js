...
let fullName = [];
runners.forEach((rep) => {fullName.push(rep["first_name"] + " " + rep["last_name"]);}); 
...
let allCaps = [];
let firstNames = [];
runners.forEach((rep) => {firstNames.push(rep["first_name"]);});
allCaps = firstNames.map(a => a.toUpperCase());
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
largeShirts = runners.filter(rep => rep["shirt_size"] === "L");
...
let amounts = [];
runners.forEach(rep => {amounts.push(rep["donation"]);});
ticketPriceTotal = amounts.reduce((accumulator, currentValue) => {return accumulator + currentValue;});
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 - show who donated more than $100 
let donatedOverAHundred = [];
donatedOverAHundred = runners.filter(rep => rep["donation"] >= 100);
console.log(donatedOverAHundred);

// Problem 2 - provide company names only
let companyNames = [];
runners.forEach((rep) => {companyNames.push(rep["company_name"]);}); 
console.log(companyNames);

// Problem 3 - returns only company name and donation
let nameDonation = [];
runners.forEach((rep) => {nameDonation.push(rep["company_name"] + " " + rep["donation"]);}); 
