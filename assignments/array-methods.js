...
let firstNamesAllCaps = runners.map(runner =>{ return runner.first_name.toUpperCase()});
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = runners.filter(shirts =>{return shirts.shirt_size === 'L'});
...
// Problem 1
let emailArray =[];
runners.forEach(emails => emailArray.push(`${emails.email}`));
console.log(emailArray);
// Problem 2
let smallShirts = runners.filter(shirts_small =>{return shirts_small.shirt_size === 'S'});
console.log(smallShirts);
// Problem 3
let lastNamesLowerCase = runners.map(lowerCase =>{return lowerCase.last_name.toLowerCase()});
