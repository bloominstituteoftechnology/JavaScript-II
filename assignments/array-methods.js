...
runners.forEach(function(list) {
  let a = list.first_name + " " + list.last_name;
  fullName.push(a);
})
...
const upperCase = runners.map(function(x) {
...
let large = runners.filter(function(runners){
...
runners.forEach(function(object){
  let a = object.donation;
  ticketPriceTotal.push(a);
})
let total = ticketPriceTotal.reduce(function(accumulator, currentValue, currentIndex, array) {
...
console.log("The amount of people that donated less that $100 is " + total)

// Problem 2

Alphabet = [];
runners.forEach(function(object){
  let a = object.first_name;
  Alphabet.push(a);
})
