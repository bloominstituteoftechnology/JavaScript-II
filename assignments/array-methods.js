// A local community center is holding a fund rasising 5k fun run
...
ticketPriceTotal = runners.reduce(function(sum, amount) {
...
// Problem 1
// The boss wants to needs the last name of each runner and the company they work at.
// solution
let lastNameAndWorkPlace = [];
runners.forEach(runner => {
  lastNameAndWorkPlace.push(
    `${runner.last_name} works at ${runner.company_name}`
  );
});
console.log(lastNameAndWorkPlace);
// Problem 2
...
let filteredNames = [];
filteredNames = runners.filter(runner => {
  return runner.first_name[0] === "B";
});
