...
let fullName = [];
  runners.forEach(function (runners){
    fullName.push(`runner: ${runners.id}: ${runners.first_name} ${runners.last_name} `)
  });
...
let largeShirts = runners.filter((runners) => {
...
let ticketPriceTotal = runners.reduce(function(totalDonate, runners){
  return totalDonate + runners.donation;
}, 0);
