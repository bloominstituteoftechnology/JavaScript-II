...
let fullNames = [];
//* runners.forEach(list = (arr) => fullNames.push(`${arr.id} ${arr.first_name} ${arr.last_name}`))
runners.forEach(function(arr) {
  fullNames.unshift(`${arr.first_name}`);
});
// runners.forEach(cbFunction);
// const cbFunction = arr =>
//   fullNames.push(`${arr.id} ${arr.first_name} ${arr.last_name}`);

//! console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
//! let firstNamesAllCaps = runners.map(arr => `${arr.first_name.toUpperCase()}`);

//! console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
//! let runnersLargeSizeShirt = runners.filter(arr => arr["shirt_size"] == "L");

//! console.log(runnersLargeSizeShirt);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let ticketPriceTotal = runners.reduce(function(accumulator, arr) {
  return accumulator + arr.donation;
}, 0);
...
// Problem 1 -- donations higher than $250
highestDoners = runners.filter(function(runner) {
  if (runner.donation > 250) {
    return `${runner.first_name} donated ${runner.donation}`;
  }
});
//! console.log(highestDoners);

// Problem 2 -- need a list of first name runners with letters 5 or less all caps
shortNameRunners = [];
runners.filter(function(runner) {
  if (runner.first_name.length < 6) {
    shortNameRunners.push(runner.first_name.toUpperCase());
  }
});
console.log(shortNameRunners);
shortNameRunnersSorted = shortNameRunners.sort();
console.log(shortNameRunnersSorted);

// Problem 3 -- make a list of all the emails

runnersEmail = runners.map(runner => runner.email);
