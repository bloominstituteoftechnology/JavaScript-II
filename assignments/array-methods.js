...
let allCaps = [];
const fNM = runners.map(x => x.first_name);
joined = fNM.join()
caps = joined.toUpperCase()
allCaps = caps.split(",")
console.log(allCaps); 
...
ticketPriceTotal= runners.reduce((sum, runner) => sum + runner.donation, 0) 
...
// Problem 1
//The race is being postponed and we need to send an Email to the runners to inform them.
let Email = [];
Email = runners.map(x => x.email);
console.log(Email)

// Problem 2
//We would like to see all the information on our runners that donated atleast $100.
let bigTipper = []
bigTipper = runners.filter(x => x.donation >= 100);
console.log(bigTipper)

// Problem 3
//Can you grab all of the company names that were represented. We would like to give a wpeech thanking the companies that these runners are representing.
let companyArray = []
runners.forEach((x) => {
companyArray.push(x.company_name)
});
