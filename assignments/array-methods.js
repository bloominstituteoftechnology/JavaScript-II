...
let fullName = [];
function cb (v, i, arr) {
    fullName.push(arr[i].first_name + " " + arr[i].last_name);
}
runners.forEach(cb);
...
function cb (v, i, arr) {
    ...
    allCaps[i] = allCaps[i].toUpperCase();
    
    return allCaps[i];
}
allCaps = runners.map(cb);
console.log(allCaps);
...
largeShirts = runners.filter(cb);
console.log(largeShirts);
// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
function cb (x, y, i, arr) {
    return x + arr[i].donation;
 }
ticketPriceTotal = runners.reduce(cb, 0);
console.log(ticketPriceTotal);
// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
//************** list of everyone who donated less than 100**********************//
let lessThanHundred = [];
function cb (v, i, arr) {
    if (arr[i].donation < 100) {
        lessThanHundred.push(arr[i]);
    }
}
runners.forEach(cb);
console.log(lessThanHundred);
// Problem 2
//******** list of email addresses because an over enthusiastic employee wants to do a phishing scam.********//
let emails = [];
function cb (v, i, arr) {
...
// Problem 3
// HR says the insurance company wants a list of all the employees with XL sizes. Just cause....
let xls = [];
let found = "";
function cb (v, i, arr) {
