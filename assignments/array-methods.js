...
let fullName = [];
runners.forEach((taco) => {
  fullName.push(taco.first_name + " " + taco.last_name);
});
...
let allCaps = [];
allCaps = runners.map(taco => taco.first_name.toUpperCase());
console.log(allCaps); 	

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
largeShirts = runners.filter(taco => taco.shirt_size === "L");
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal = runners.reduce((tally, taco) => tally + taco.donations, 0);
...
// Problem 1
// The boss wants to know what companies have convinced their employees to run in their off time.
let corporateSlave = [];
corporateSlave = runners.filter(taco => taco.company_name);
console.log(corporateSlave);
// Problem 2
// The boss wants both the runners first and last name in upper caps because his eye sight is fading and he won't get glasses. 
let bossIsBlind = [];
bossIsBlind = runners.map(taco => taco.first_name.toUpperCase() + " " + taco.last_name.toUpperCase());
console.log(bossIsBlind); 
// Problem 3
// The boss thought that pulling the arrays to see what companies donated their people was too much reading. He wan't the company name along with the runners name instead. 
let ownerAndRunner = [];
runners.forEach((taco) => {
  ownerAndRunner.push(taco.company_name + ": " + taco.first_name + " " + taco.last_name);
});
console.log(ownerAndRunner);