...
let allCaps = [];
runners.map((runners, first_name) => 
...
{return runner.donation + total ;}, 0)
...
// Problem 1
//find all the companies that donated more than $250 and alphebetize them.
let highestDonation = [];
highestDonation = runners.filter(runner => runner.donation > 250).map(runner => runner.company_name);
highestDonation = highestDonation.sort();
console.log(highestDonation);

// Problem 2
//alphebetize last names using map.
let orderNames = [];
let sum = runners.map((runners, last_name) => {
orderNames.push(runners.last_name)
...
return array.company_name === 'Wordtune';
