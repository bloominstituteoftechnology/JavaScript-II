...
runners.forEach(function (runner) {
    //Grab first and last name and push into fullName
    //Use a string literal so the first and last name remain as one index and dont get pushed in one at a time and each have an index
...
runners.map(function (runner) {
    allCaps.push(`${runner.last_name}, ${runner.first_name.toUpperCase()}`);
});
console.log(allCaps);
...
runners.filter(function (runner) {
    return ticketPriceTotal.push(runner.donation)
});
console.log(ticketPriceTotal.reduce(function (a, b) {
    return a + b
}));
...
// Problem 1
//Create a mailing list to send out thank yous for the donation and attending the event.  Filter by email and company and use string literals to generate template emails.

// Problem 2
// Use map to create a list of companys to submit to the newspaper for press and public relation purposes.

// Problem 3 
//Generate a database of users for future reference.