...
runners.forEach((runners) => {
    let firstName = runners.first_name;
    let lastName = runners.last_name;
    return fullName.push(`${firstName} ${lastName}`);
})
...
let allCaps = runners.map(function(makeAllCaps) {
    return makeAllCaps.first_name.toUpperCase()
})
