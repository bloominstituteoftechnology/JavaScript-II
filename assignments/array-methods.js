...
// Problem 1
//Skinix wants to know how much its employees donated.

let Skinix = runners.filter((data) => {
  return data.company_name === 'Skinix';
});
//console.log(Skinix);

let donationsSkinix = Skinix.reduce((theReducer, item) => {
  return theReducer += item.donation;
}, 0);
console.log(donationsSkinix);



// Problem 2
//Divanoodle wants to know how many of its employees participated.
let Divanoodle = runners.filter((data) => {
  return data.company_name === 'Divanoodle';
});
console.log(Divanoodle.length);



// Problem 3
//We want to give prizes to people who donated over $100. Make an array of these people.

let highRollers = runners.filter((data) => {
