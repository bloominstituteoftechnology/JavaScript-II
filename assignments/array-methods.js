...
// Problem 1
let runnersCompanies = runners.map(function(runner) {
  return runner.company_name;
});

console.log(runnersCompanies);

// Problem 2

runners.forEach(function(item, index) {
  console.log(`${item.id} ${item.first_name} ${item.last_name} from ${item.company_name} donated: $${item.donation}`);
})

// Problem 3

let over150Donations = runners.filter(function(runner) {
  return runner.donation > 150;
});

console.log(over150Donations);