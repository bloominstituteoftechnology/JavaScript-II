...
let allCaps = fullName.map( name => name.toUpperCase())
console.log(allCaps); 
...
const tallyDonations = (total, runner) => total + runner.donation;
let ticketPriceTotal = runners.reduce(tallyDonations, 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 compile emails into a string
let emails = runners.reduce( (emails, runner) => emails + runner.email + ', ' + '\n', '' );
console.log(emails);

// Problem 2 count the num of people from each company
const countProp = (arr, prop) => {
  let counterObj = {};
  arr.forEach( obj => {
    if (!counterObj.hasOwnProperty(obj[prop])) {
      counterObj[obj[prop]] = 1; 
    }
    counterObj[obj[prop]]++;
  });
  return counterObj;
}
const counterStr = tuple => `${tuple[0]} has ${tuple[1]} entries.`;
console.log(
...
console.log(
  Object.entries(countProp(runners, 'shirt_size'))
  .map( t => `${t[1]} ${t[0]}\'s` ).join('\n')
);
