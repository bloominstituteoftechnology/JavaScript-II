...
console.log('list of full names: ', fullName);
...
runners.map(item => allCaps.push(item.first_name.toUpperCase()));

console.log('to uppercase: ', allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];

largeShirts.push(runners.filter(item => item.shirt_size === 'L'))
// console.log(runners.filter(item => item.shirt_size === 'L'));
// largShirts = runners.filter(item => item.shirt_size === 'L' );
console.log('people with large shirts: ', largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal.push(runners.reduce((total, donation) => total + donation.donation, 0));
...
// Problem 1
// give a label to donors based on the amount they've donated. ie: > $1000 === Gold Sponsor. < $100 === 'Partner'
