...
let allCaps = [];
allCaps = runners.map(function(run) {
    return run.first_name.toUpperCase();
})
console.log(allCaps); 
...
let largeShirts = [];
largeShirts = runners.filter(function(run) {
    return run.shirt_size === 'L';
})
...
ticketPriceTotal = runners.reduce(function(total, run) {
