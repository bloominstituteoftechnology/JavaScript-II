...
let fullName = [];
const ppl = runners.forEach(function(runners){
    return fullName.push(runners.first_name + " " + runners.last_name);
})
...
let allCaps = [];
const nombres = runners.map(function(runners){
...
let largeShirts = [];
const shirtNumber = runners.filter(function(runners){
    return largeShirts.push(runners.shirt_size == L);
})
