...
// Problem 1, lists donors who gave over 100

let whales = runners.filter((data) => {
    return data.donation > 100;
})

console.log(whales);

// Problem 2, sort runners by last name

let runnersSorted = runners.map((item) =>{
    return item.last_name;
})

console.log(runnersSorted.sort());

// Problem 3, donation avg

let donoTotal = runners.reduce((theReducer, item) =>{
    return theReducer +=item.donation;
}, 0)
let donoAvg = donoTotal / runners.length;
console.log("Average donation per runner: " + donoAvg);
