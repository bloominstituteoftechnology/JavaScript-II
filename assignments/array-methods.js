...
console.log('Runners who ordered large shirts:')
...
// you could solve given the data set and the 5k fun run theme.  Try to solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
// Which company gave the largest donation?
const maxDonation = (max, current, index, array) => {
	if(current.donation > max[0]){
		return [current.donation, current.company_name];
	}
	else {
		return max;
	}
};
let biggestDonor = runners.reduce(maxDonation, [0, ''])
console.log(`The largest donation was $${biggestDonor[0]} from ${biggestDonor[1]}`);

// Problem 2
// Which companies donated more than $200? They will be listed as "sponsors".
let sponsors = runners.filter(runner => runner.donation > 200).map(runner => runner.company_name);
console.log(`Our sponsors are: ${sponsors}`);

// Problem 3
// Collect all email addresses to send a thank-you email to each runner.
let emails = {}
runners.forEach((runner) => {
	emails[runner.first_name] = runner.email;
})
