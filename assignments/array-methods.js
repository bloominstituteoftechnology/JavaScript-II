...
ticketPriceTotal = runners.reduce((acc,x) => {return acc += x.donation},0)
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 - Find donations > $100
let bigDono = [];
bigDono = runners.filter(x => x.donation >= 100);
console.log(bigDono);

// Problem 2 - find all .org email
let nonProfit = [];
nonProfit = runners.filter(x => x.email.includes('.org'));
console.log(nonProfit);

// Problem 3 - give all L-sized runners XL shirts instead, follow-up on Challenge 3
let sizeUp = [];
sizeUp = runners.map(x => x.shirt_size.replace('L' , 'XL'));
console.log(sizeUp);
