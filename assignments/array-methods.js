...
let fullName = [];
let newFullName = runners.forEach(runners => {
    console.log(runners["first_name"] + " " + runners["last_name"])
});
    
// let fullName = [];

// let newFullName = runners.map(runners=> {
...
let newAllCaps = runners.map(runners=> {

    return runners["first_name"].toUpperCase();

});

allCaps.push(newAllCaps);
...
let theRighOnes = runners.filter(runners => runners["shirt_size"] === "L")

largeShirts.push(theRighOnes);
...
const charitableDonas = runners.reduce((theReducer, item) => {
    
    return theReducer += item.donation;},0);

    ticketPriceTotal.push(charitableDonas);
