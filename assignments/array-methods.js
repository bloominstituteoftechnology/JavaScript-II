...
let allCaps = runners.map(function(upper) {
    console.log(upper.first_name.toUpperCase());
});
...
let ticketPriceTotal = runners.reduce(function(totalDon, currentValue) {
    return totalDon + currentValue.donation;
}, 0);
...
let nameAndEmail = runners.forEach(function(contact) {
    console.log(contact.first_name + " " + contact.last_name + " " + contact.email);
});

// Problem 2

let companyCaps = runners.map(function(company) {
    return company.company_name.toUpperCase();
});
