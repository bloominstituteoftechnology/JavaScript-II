...
runners.forEach(function(items){
  return fullNames.push(`${items.first_name} ${items.last_name}`) //get the firts and last name and push it to fullNames array
  });
...
runners.map(function(allNames){
  return firstNamesAllCaps.push(`${allNames.first_name.toUpperCase()}`)
});
...
runnersLargeSizeShirt = runners.filter(function(runnersSizeLarge){
  return runnersSizeLarge.shirt_size === "L";
})
...
// Problem 1
// An email need to be send to all the runners to let them now the time of event wil change. get all emails
let emails = []
runners.forEach(function(getEmails){
emails.push(getEmails.email);
});
console.log(emails);
