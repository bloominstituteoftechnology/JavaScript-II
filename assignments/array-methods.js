...
console.log(ticketPriceTotal.reduce(function (acc, val, ind, arr) {
    return acc + val;
}));
...
// Problem 1 - Provide full name, email, and current donation amount for any runners that have donated less than $100 to reach out for a bigger donation.

let reachOut = [];
    runners.filter(function(runner) {
        if (`${runner.donation}` < 100) {
            reachOut.push(`${runner.first_name} ${runner.last_name} has donated $${runner.donation} and can be emailed at: ${runner.email}.`)
        }
    })

console.log(reachOut);

// Problem 2 - Categorize runners into tiers based on how much they've donated.

let runnerTierBronze = [];
let runnerTierSilver = [];
let runnerTierGold = [];

    runners.filter(function(runner) {
        if (`${runner.donation}` < 100) {
            runnerTierBronze.push(`${runner.first_name} ${runner.last_name} has donated $${runner.donation} Tier: Bronze.`);
            
        } else if ((`${runner.donation}` >= 100) && (`${runner.donation}`<=199)) {
            runnerTierSilver.push(`${runner.first_name} ${runner.last_name} has donated $${runner.donation} Tier: Silver.`)
        } else {
            runnerTierGold.push(`${runner.first_name} ${runner.last_name} has donated $${runner.donation} Tier: Gold.`)
        }
    })

console.log(runnerTierBronze);
console.log(runnerTierSilver);
console.log(runnerTierGold);
    
// Problem 3 - Alphabetize the names of the runners

let unsorted = [];

runners.map(function(runner) {
    unsorted.push(runner.first_name);
