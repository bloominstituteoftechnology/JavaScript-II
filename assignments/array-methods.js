...
runners.forEach(function(currentValue){
  let names = currentValue.first_name + ' ' + currentValue.last_name;
...
let runnersLargeSizeShirt = runners.filter(function(currentValue){
...
// Problem 1 Find out which which company is participating/donating to the 5k fun run.
let companiesDon = [];

runners.forEach(function(currentValue){
  let companyName = currentValue.company_name;
  companiesDon.push(companyName);
});

console.log(companiesDon);
...
// Problem 3 The organizer had already put down a huge dontaion before donations were being accepted.
let donationStart = runners.reduce(function(accumulator, currentValue){
  return accumulator + currentValue.donation;
}, 15000);

console.log(donationStart);
