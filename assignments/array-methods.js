...
// The large shirts won't be available for the event due
//to an ordering issue.  Get a list of runners with large sized
// shirts so they can choose a different size. Return an array named
  //largeShirts that contains information about the runners that have
...
runners.filter((runner) => {
   if (runner.shirt_size === "L") {
    largeShirts.push(`${runner.first_name} ${runner.last_name}`);
   }
  })
...
let ticketPriceTotal = runners.reduce(function(value, currentEntry) {
   return value + currentEntry.donation;
  }, 0);
