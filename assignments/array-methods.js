...
// Problem 1
// The director wants a chart thats easy to read who donated what for the crowd to see. retun the name of the company and the amount they donated in a new array.
let companyDono = [];

runners.forEach(function(runner){
    companyDono.push(runner.company_name + ", "+runner.donation);
});

console.log(companyDono);

// Problem 2
let sortedCompanies = []

runners.forEach(function(runner){
   return sortedCompanies.sort().push(runner.company_name);
})
...
runners.forEach(function(runner){
   return emailList.sort().push(runner.email);
})
