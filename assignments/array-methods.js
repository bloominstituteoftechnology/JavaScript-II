...
runners.forEach(createFullName);
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
const cbAllCaps = (item) => {
  return result = item.first_name.toUpperCase();
}
let allCaps = runners.map(cbAllCaps);
console.log(allCaps); 
...
// Problem 1: giving anyone that made their donation over $100 a shoutout!
let bigDonator = [];
const cbBigDonator = (item) => {
