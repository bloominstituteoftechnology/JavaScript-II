...
// Problem 1
// Each runner gets a shirt with their company name and initials printed on it. Return an array of each runner's company name, initials, and shirt size to send to the screen printers and log the result.
let shirtOrder = [];
runners.forEach(runner => shirtOrder.push({
  company: runner.company_name,
  initials: runner.first_name[0] + runner.last_name[0],
  size: runner.shirt_size
}));
console.log(shirtOrder);

// Problem 2
// While we're at it, let's make a simple list of the quantity of shirts needed per size.
let shirtSizes = {'XS': 0, 'S': 0, 'M': 0, 'L': 0, 'XL': 0, '2XL': 0, '3XL': 0};
runners.forEach(runner => shirtSizes[runner.shirt_size]++);
console.log(shirtSizes);

// Problem 3
// The organizers would like to publicly shame any businesses that donated less than $100. Create an array of the miserly participants and log the result.
let stingyBastards = []
runners.filter(runner => runner.donation < 100).forEach(runner => stingyBastards.push(runner.company_name));
console.log(stingyBastards);