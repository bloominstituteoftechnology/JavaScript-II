...
let fullName = runners.forEach(arr => console.log(`${arr.first_name} ${arr.last_name}`));
...
// Problem 1
let shirtsNeeded = runners.filter(
  runner =>
    runner.shirt_size === "L" ||
    runner.shirt_size === "M" ||
    runner.shirt_size === "S" ||
...
// console.log(shirtsNeeded.length);

// Problem 2
// employees with even id numbers
let evenSteven = runners.map(runner => {
    if (runner.id % 2 == 0) {
        return runner;
    }
})
...
// Problem 3
// company list
let companyList = runners.map(runner => runner.company_name);
