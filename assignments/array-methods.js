...
// Scroll to the bottom of the list to use some advanced array methods to help
// the event director gather some information from the businesses.
...
// the 5k fun run theme.  Try to solve 3 unique problems using one or many of
// the array methods listed above.

// Problem #5.1
// Gather the e-mails on all the small sized runners using their shirt size.

let smallShirtRunnersEmails = [];
runners.filter(runner => runner.shirt_size === 'S').forEach(runner => {
  smallShirtRunnersEmails.push(runner.email);
  smallShirtRunnersEmails.push(runner.first_name + ` ` + runner.last_name)
})

console.log(smallShirtRunnersEmails);
