...
runners.forEach(function(id) {
  let combined = id.first_name + ' ' + id.last_name;

  return fullName.push(combined);
})
...
runners.map(function(id){
  let capsName = id.first_name.toUpperCase();

  return allCaps.push(capsName);
})
...
const filteredShirts = runners.filter(runner =>
  {
...
const sum = donations.reduce((total, amount) => total + amount);
