...
let allCaps = [];
runners.map(function(upperCase) {
  return allCaps.push(upperCase.first_name.toUpperCase())
})
console.log(allCaps);
...
let largeShirts = runners.filter(function(sizes) {
  return sizes.shirt_size === "L";
})
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce(function(theReducer, item) {
...
// Problem 1
const donattionFilter = runners.filter(runner=>{
    if (runner.donation >100) {
      return true;
    }
  })

console.log(donattionFilter);
// Problem 2
const filterEvenValue = runners.filter(value => {
  return value.donation % 2===0;
})

console.log(filterEvenValue);

// Problem 3
const donationReduce = runners.reduce(function(theReducer, item){
  return theReducer + item.donation;

}, 0);
const donationAverage = donationReduce / runners.length;
console.log(donationAverage); // 140.86
