...
let allCaps = fullName.map(({first_name, last_name}) => (
    {
...
let largeShirts = runners.filter(({shirt_size}) => {
    if (shirt_size === "L") return true
})
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((accum, {donation} ) => (
    accum + donation
), 0)
...
let govEmails = emails.filter(email => email.includes(".gov"))

console.log("Government Emails:", govEmails)

// Problem 3

let emailDomains = emails.map(email => {
