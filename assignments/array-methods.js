...
let ticketPTotal = runners.reduce((donation, runners) => {
...
let largeDonors = runners.filter((runner) => {
...
runners.forEach(function(runners) {
    emailListWithNames.push(runners.first_name + ", " + runners.email);
});
...
runners.filter((runners) => {
    if (runners.company_name === 'Wordtune') {
        return matchedDonations.push(runners.first_name + " " + runners.last_name + ", " + runners.company_name + ", $" + runners.donation);
    }
});
