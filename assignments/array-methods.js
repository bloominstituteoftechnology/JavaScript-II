...
const filterLargeShirts = runners.filter((person) => {
  return person.shirt_size == "L";
});

largeShirts.push(filterLargeShirts)
...
const reduceDonation = runners.reduce((total, person) => {
  return total += person.donation;
}, 0);

ticketPriceTotal.push(reduceDonation)
