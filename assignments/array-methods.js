...
let fullName = runners.map(runner => `${runner.first_name} ${runner.last_name}`);
// NON ARROW METHOD
...
let largeShirts = runners.filter(large => large.shirt_size === "L"); 
// NON ARROW METHOD
...
// let ticketPriceTotal = runners.reduce(function(total, person) {
...
// Problem 1
// Sort each runner by who donated the most. Then thank them by saying their first name, last name, company name, and donation amount.
let idLessThan25 = runners.filter(runner => (runner.id <= 25));
console.log (idLessThan25);


// Problem 2
// Filter runners who have donated more than $100, compute the tax of 9% of each donation in a new array, and add it all together to find out how much tax has to be paid in total.
let tax = runners
    .filter(runner => runner.donation >= 100)
    .map(runner => runner.donation * .09)
    .reduce((a, b) => a + b, 0);
console.log (`${tax} has to be paid in total for taxes.`);


// Problem 3
// Filter out the each person with an odd ID number.
let sortedDonations = runners
    .sort((a, b) => (a.donation < b.donation) ? 1 : -1)
    .map(runner => `Thank you ${runner.first_name} ${runner.last_name} from ${runner.company_name} for donating $${runner.donation}.`);
