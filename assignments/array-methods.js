...
runners.forEach(function(item) {
  fullName.push(item.first_name) && fullName.push(item.last_name);
});
...
const fNameCb = (item) => {
    let firstName = item.first_name.toUpperCase();
    return firstName;
}
allCaps = runners.map(fNameCb);
...
shirts = runners.filter(filterLarge);

shirts.forEach(function(item) {
  largeShirts.push(item.first_name);
});
...
console.log(ticketPriceTotal.reduce(donoAdd));

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
// List emails for each of the runners
let runnerEmail = [];

runners.forEach(function(item) {
  runnerEmail.push(item.email);
});

console.log(runnerEmail);

// Problem 2
// There is a raffle! BUT ITS RIGGED! It is selected based on the length of the runners last name!
let raffleWinner = [];

const winner = (item) => {
  if (item.last_name.length === 9){
    let runner = `${item.first_name}`;
    return runner;
  }
}
raffleWinner = runners.filter(winner);

console.log(raffleWinner);
console.log(raffleWinner);

// Problem 3
// THE DIRECTOR IS STILL DRUNK WITH POWER! Convert all company names into lowercase!
let allCapsCompany = [];

const cNameCb = (item) => {
  let companyName = item.company_name.toLowerCase().toString();
  return companyName;
}
allCapsCompany = runners.map(cNameCb);
