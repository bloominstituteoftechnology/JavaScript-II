...
// Problem 1
// Return the average runner's donation
let averageDonation = runners.reduce(function(acc, runner) {
    return acc += runner.donation; //?
}, 0) / runners.length;
averageDonation

// Problem 2
// Return the total donation of runners whose shirt size is M. 
let mSizedDonation = runners.filter(function(runner) {
...
// Problem 3
// Return runners sorted by alphabetical order. 
let sortedRunners = runners.map(function(runner) {
    return runner.first_name + " " + runner.last_name;
})

console.log(sortedRunners.sort());
