...
// Problem 1 Only returning the first name and email of the runner

let nameAndEmail = runners.map (runner => {

    return `${runner["first_name"]} and ${runner["email"]}`

});

console.log(nameAndEmail);

// Problem 2 Only people who donating more than 150 dollars

let over = runners.filter( runner => {
    return runner["donation"] > 150 ;
});

console.log(over);

// Problem 3 adding a number time property to their object literal

let time = runners.forEach(runner => {
    return runner["time"] = 0;
});
