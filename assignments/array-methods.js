...
runners.map((runner) => ticketPriceTotal.push(runner.donation));
...
// Problem 1
// Set up email array to thank the runners for donating
emailList = [];
runners.map((runner) => {
  emailList.push(
    `To: ${runner.email}
    From: 5kfunrun@funrun.com
    Subject: Contribution to 5k Fun Run
    Thank you for your contribution, ${runner.first_name} ${runner.last_name}!`
  )
});
