...
let ticketPriceTotal = runners.reduce(function(totalMoney, item)
{
    return totalMoney + item.donation;
}, 0);
...
// Problem 1
// Any business that donates $200 or more has Gold Donor status. Which businesses are Gold Donors?

let goldDonor = runners.filter((data) => {
    return data.donation > 200;
})
console.log(goldDonor);


// Problem 2
//The runner's bib numbers will be their id numbers - pair each last name with a bib number.

let bibNum = [];

runners.forEach(function(runner){
  bibNum.push(runner.last_name + ' ' + runner.id);
});
...
// Problem 3
// The race organizers are writing thank you notes to participating businesses. Which business donated the most money?

let topDonor = runners.reduce(function(currentDonor, item)
{
    return (currentDonor.donation > item.donation) ? currentDonor : item
})


console.log(topDonor.company_name);
