...
let allCaps = [];
runners.map(function(___, index, arr) {
    allCaps.push(arr[index].first_name.toUpperCase())
})
console.log(allCaps);
...
let largeShirts = runners.filter(sizeFiltered);
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let donations = runners.map(function(___, index, arr) {
    return arr[index].donation;
})
let ticketPriceTotal = donations.reduce(function(acc, value) {
    return acc + value;
})
...
// Problem 1
//filter less than 100 donation
let under100 = runners.filter(function(___, index, arr) {
    if (arr[index].donation < 100) {
        return true;
    }
    return false;
})
...
console.log(under100)
    // Problem 2
    //list company names in alphabetical order 
let companies = runners.map(function(___, index, arr) {
    return arr[index].company_name;
}).sort();
console.log(companies);
// Problem 3
//as seen in companies array, most common company is Skinix. Filter runners by Skinix company.
let skinixEmployees = runners.filter(function(___, index, arr) {
    if (arr[index].company_name === "Skinix") {
        return true;
    }
    return false;
})
