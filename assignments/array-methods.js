...
ticketPriceTotal = runners.reduce(function(reducer, item) {
...
// Problem 1: find list of runners that donated more than  150. Provide sorted list of array
const bigDonors = runners.filter(function(item) {
  return item.donation > 150;
});
console.log(bigDonors);

// Problem 2: pass a string to a callback function with personalized message which includes introduction and runner name

runners.forEach(function(item) {
  sendEmail(item.first_name, item.donation);
});

function sendEmail(name, donation) {
  string =
    "Hi " +
    name +
    "\n" +
    ...
    donation +
    ". Welcome to our 5k run!";
  console.log(string);
}

// Problem 3: Average donation

let total = runners.reduce(function(reducer, item) {
  return (reducer += item.donation);
}, 0);
