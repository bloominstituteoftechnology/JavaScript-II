...
const allCaps = runners.map( function(cb){
...
const largeShirts = runners.filter((data) =>{
...
const ticketPriceTotal = runners.reduce((totals, runners) => totals + runners.donation,0);
...
// Problem 1
//Placing all runners into alphabetic order so starting positions can be assigned and the data is better organized.
const alphabetic = runners.map( function(cb){

  return cb.first_name;
})
console.log(alphabetic.sort())
// Problem 2
//provide an array with the last name and donation amount combined in alphabetic order for accounting
let accounting = [];
runners.forEach((element) => {
  accounting.push(element.first_name + ' ' + element.donation);})
  console.log(accounting.sort());
// Problem 3
// provide free gold lettering on t shirts for those that donated over 200 dollars
const goldletterting = runners.filter((data) =>{
  return data.donation > 200;
});
console.log(goldletterting);
