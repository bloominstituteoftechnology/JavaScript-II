...
ticketPriceTotal=runners.reduce((a,b)=>a+b.donation,0)
...
// Problem 1
// The company wants to add each runners e-mail id inside a new array called eMail . Using forEach create a new array with all the users first name and e-mail id inside 

let eMail=[];
runners.forEach(function(item){
    eMail.push(item.first_name,item.email);
})

console.log(eMail);

// Problem 2
// Using map change all the name of the companies in all caps . 

let updatedNames=[];
runners.map(function(name){
    updatedNames.push(name.company_name.toUpperCase())
})
...
let smallShirt =[];
runners.filter(function(size){
    if(size.shirt_size==='S'){
        smallShirt.push(size)
    }
})
