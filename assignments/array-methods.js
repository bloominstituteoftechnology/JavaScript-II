...
let largeShirts = [];
largeShirts.push(runners.filter(function(item){
...
let Total = runners.reduce(function(total, item){
...
// Problem 1 - Create an Array Containing runner name and email in object form

let arrObj1 = [];
arrObj1.push(runners.map((item) => {
    return {"first_name": item.first_name,"last_name":item.last_name,"email":item.email};
}));
console.log(arrObj1);
