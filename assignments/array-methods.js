...
let firstNamesAllCaps = runners.map((currentItem) => {
  currentItem.first_name = currentItem.first_name.toUpperCase();
  return currentItem;
});
...
let runnersLargeSizeShirt = runners.filter((currentItem) => {
  return currentItem.shirt_size === "L"
});
...
// Problem 2
let runnersDonations = runners.filter ((currentItem) => {
  return currentItem.donation < 100
});
console.log(runnersDonations);

// Problem 3
let emailUpperCase = runners.map((currentItem) => {
  currentItem.email = currentItem.email.toUpperCase();
  return currentItem;
});
