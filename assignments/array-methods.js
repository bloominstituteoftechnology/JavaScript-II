...
let allCaps = [];
allCaps = runners.map(function(elem) {
...
ticketPriceTotal = runners.reduce(function(total, currentValue) {
...
// Problem 1
let emails = [];
emails = runners.map(function(elem) {
	return elem.email;
});
console.log(emails);
...
// Problem 2
let nameStartsWithS = [];
nameStartsWithS = runners.filter(function(elem) {
	return elem.first_name[0] === 'S' || elem.first_name[0] === 's';
});
console.log(nameStartsWithS);

// Problem 3
let nameEmail = [];
runners.forEach(function(elem) {
	nameEmail.push(`${elem.first_name} ${elem.last_name} ${elem.donation}`);
});
