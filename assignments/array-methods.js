...
// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the
// director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = [];
...
let ticketPriceTotal = runners.reduce((total, totalDonation) => {
    return total += totalDonation.donation;}, 0);
...
// Problem 1 - Create an email list to send 5k updates to participants 
let emailList= [];
runners.forEach(function(item) {
item = {
    'first_name': item.first_name,
...
// Problem 2 Need a list of top donors to recognize on website 
let topDonors = runners.filter((totalDonation) => {
	return totalDonation.donation >= 250;
})
console.log(topDonors);

// Problem 3 Skinix is a big corporate donor and the 
//5K wants to have a special cocktail hour for their employees. Make a contact lists.
let skinixEmployees= runners.filter((employees) => {
	return employees.company_name == "Skinix";
})
console.log(skinixEmployees); 
//apparently this will be an awkard 3 person cocktail hour 