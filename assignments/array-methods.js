...
let fullName = [];
// get full names into fullName array by grabbing and combing their first and last names for their running bibs then push it into a new array. 
// let splitNames = runners.forEach(function(name) {
// fullName.push`${name.first_name} ${name.last_name}`;
// });
let splitNames = runners.forEach(name => fullName.push`${name.first_name} ${name.last_name}`);
console.log(fullName)

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
// let allCaps = [];
// set first names to caps because director is power hungry.
let allCaps = runners.map((peeps) => {
    return peeps.first_name.toUpperCase();
  });
console.log(allCaps); 
...
let largeShirts = runners.filter(shirtSize => {return shirtSize.shirt_size === "L"});
...
// Problem 1
let lastNamesSorted = [];
    let filtLastNamesAlph = runners.filter(sname => lastNames.push`${name.last_name}`)
        lastNamesSorted = filtLastNamesAlph.sort();
    console.log(lastNamesSorted);

// Problem 2
//grab emails fir everyone with the large shirts to tell them to change shirt sizes
let emails = [];
emailers = runners.filter(emailee => {
  return emailee.shirt_size
});
