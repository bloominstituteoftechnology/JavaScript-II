...
let fullName = [];
const fullnameCB = runners.forEach((item, index, array) => {
    fullName.push(array[index].first_name +' '+ array[index].last_name);
})
...
let allCaps = [];
const allCapsCB = runners.map((item, index, array) => {
...
let largeShirts = [];
const largeShirtsCB = runners.filter((item, index, array) => {
...
ticketPriceTotal.push(runners.reduce((acc, item) => acc + item.donation))
