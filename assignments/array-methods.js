...
// Problem 1
// Give me a list of all the companies and how many runners each one has in the event, sorted by most to least runners per company.
let companies = [];
runners.forEach(runner => companies.push(runner.company_name));
companies.sort();

let results = {};
companies.forEach((company) => results[company] = (results[company] || 0) + 1);

resultsSorted = Object.keys(results).sort(function(a, b){return results[b] - results[a]});

let finalAnswer = [];
resultsSorted.forEach(resultSorted => {
    if (results[resultSorted] === 1) {
        finalAnswer.push(resultSorted + " has " + results[resultSorted] + " runner");
    } else {
        finalAnswer.push(resultSorted + " has " + results[resultSorted] + " runners")
    }
});
...
// Problem 2
// The director continues to be drunk with power and now needs to have a list of all the donations written in USD format. Convert each donation and log the result.
let donations = [];
runners.map(runner => donations.push("$" + runner.donation.toFixed(2)));
console.log(donations);

// Problem 3
// How many of each shirt size were ordered?
let sizes = [];
runners.forEach(runner => sizes.push(runner.shirt_size));
let sizesObject = {};
sizes.forEach((size) => sizesObject[size] = (sizesObject[size] || 0) + 1);
