...
let ticketPriceTotal = runners.reduce(
  (accumulator, currentValue) => accumulator + currentValue.donation,
  0
);
...
// Problem 1 email validation for @ symbol
const email = runners.map(function(index) {
  if (index.email.includes("@")) {
    return `${index.first_name} ${index.last_name} provided a valid email`;
  } else {
    return `${index.first_name} ${
      index.last_name
    } has not provided a valid email`;
  }
});
console.log(email);
...
const name = fullName.map(function(index) {
  if (!/[^a-zA-Z ']/.test(index)) {
    return `${index} is a valid name`;
    console.log("True name");
  } else {
    return `${index} is not a valid name`;
  }
});
console.log(name);
