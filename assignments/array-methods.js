...
const ticketPriceTotal = runners.reduce((totalTicket, currentValue) => {
    return totalTicket + currentValue.donation;
},0)
...
// Problem 1
// Get all runners emails to send them info on the race
...
runners.filter(currentValue => {
    if(currentValue.company_name === "Gigashots"){
        gigashotEmployees.push(currentValue)
    }
})

for(let i in runners){
    for(let x in gigashotEmployees){
        if(runners[i].id == gigashotEmployees[x].id){
            runners[i].donation += 100;
        }
    }
}
...
// Problem 3
// company added a 5000 donation to the race, add this to the start value using reduce
