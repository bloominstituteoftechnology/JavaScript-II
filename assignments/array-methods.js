...
// Problem 1
//Last Name, First Name sorted alphabetically for a roll sheet
let sortedRunners = runners.map( runner => { return `${runner.last_name}, ${runner.first_name}` }).sort();
console.log(sortedRunners);

// Problem 2
//list of participating businesses
let theBusinesses = [];
runners.forEach( runner => { if(!theBusinesses.includes(runner.company_name)) {
  theBusinesses.push(runner.company_name);
  }
});
theBusinesses.sort();
