...
let allCaps = runners.map(e=>{
    return`${e.first_name.toUpperCase()}`
});
...
ticketPriceTotal.push( runners.reduce((acc,cur,idx,src)=>{
...
let  topThreeContributors= runners.filter((e, index)=>{
    return index < 3;
})
console.log(topThreeContributors) 
// Problem 3
let  shirtCount = {};
runners.forEach(e=>{
    if(shirtCount.hasOwnProperty(e.shirt_size)){
        shirtCount[e.shirt_size]++;
    }else{
        shirtCount[e.shirt_size]= 1;
    }
})
