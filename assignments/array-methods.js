...
// Problem 1
// We need the total amount of people that donated 40 dollars or more, because only those will people will be receiving medals for completing the race.
let totalAmountOfMedals = 0;
totalAmountOfMedals = runners.filter(item => item.donation >= 40).length;
console.log(totalAmountOfMedals);

// Problem 2
// We need a list of unique companies participating in the race, ordered alphabetically
listOfCompanies = [];
listOfCompaniesSorted = [];
listOfCompanies = runners.map(item => item.company_name);
listOfCompaniesSorted = listOfCompanies
  .filter((item, index) => listOfCompanies.indexOf(item) === index)
  .sort();

console.log(listOfCompaniesSorted);

// Problem 3
// We need want to give a special reward to those that donated 4% or more of the total donations. Its okay to use previous ticketPriceTotal, return an array of objects.
let deepWallets = [];
deepWallets = runners.filter(item => item.donation / ticketPriceTotal >= 0.04);
console.log(deepWallets);
