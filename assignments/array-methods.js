...
// Problem 1
// People whove donated over 200 need to be notified about their special rewards return an array of the emails of people who donated over 200
let topDonaters = runners.filter(runner => runner.donation > 200).map((runners => runners.email));
console.log(topDonaters);


// Problem 2
// Need to reprint runners names on all SMALL t-shirts return and array with runners firts and last name that have a size S
let reprint = runners.filter(runners => runners.shirt_size === 'S').map(runners => `${runners.first_name} ${runners.last_name}`);
console.log(reprint);

// Problem 3
// The drunken director wants more $$ so to let more higher donation runners participate since theres a 50 runner cap return an array of objects with email, name, donation with anyone who donated less than $50 so we can notify them that they will no longer participate
let extraRunners = runners.filter(runners => runners.donation < 50).map(runners => { return { name: `${runners.firs_name} ${runners.last_name}`, email: runners.email, donation: runners.donation}});
console.log(extraRunners);