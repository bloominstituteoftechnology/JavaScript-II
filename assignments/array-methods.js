// A local community center is holding a fund rasising 5k fun run and has invited 50 small businesses 
// to make a small donation on their behalf for some much needed updates to their facilities.  
// Each business has assigned a representative to attend the event along with a small donation.

// Scroll to the bottom of the list to use some advanced array methods to help the event director gather 
// some information from the businesses.
...
// The event director needs both the first and last names of each runner for their running bibs.  Combine both the first 
...
const bigFirstNames = runners.map(function(runner){
    return runner.first_name.toUpperCase();
});

console.log(bigFirstNames); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with 
// large sized shirts so they can choose a different size. Return an array named largeShirts that contains information 
// about the runners that have a shirt size of L and log the result
let largeShirts = [];

// pretty sure this is not the correct usage
...
const ticketPriceTotal = runners.reduce(function(sum, runner) {
...
// you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using 
// one or many of the array methods listed above.

// Problem 1
// Find all donations greater than $200 and put their names in an array for Honoring.
let bigDonors = [];

runners.forEach(runner => {
    if (runner.donation > 200) {
        bigDonors.push(runner.last_name);
    }
});
...
// Problem 2
// Find all companies with a 'z' or 'Z' in the name and return their info in a new array.

const zedCompanies = runners.filter(runner => {
    return runner.company_name.includes('Z')
});

console.log(zedCompanies);


// Problem 3
// Add up all donations smaller than $50 and log the sum.

const smallFry = runners.reduce(function(sum, runner){
