...
let anotherWay = runners.map(runner => {
  fullName.push(runner.first_name.toUpperCase());
});
...
let testArray = [1,5,4];
let totalArr = testArray.reduce((a, b) => {
  return a + b;
})
console.log(totalArr);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of a potential problems you could solve given the data set and the 5k fun run theme.  Try to solve 3 unique problems using one or many of the array methods listed above.
