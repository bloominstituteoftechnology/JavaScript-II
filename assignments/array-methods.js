...
ticketPriceTotal = runners.reduce((ticketSales, runner) => {return ticketSales += runner.donation}, 0);
...
// Problem 1
// Make an email list for the marketing team
let emailList = function(runners){
    return runners.map(runner => runner.email);
    
}
console.log(emailList(runners).sort());

// Problem 2
// add up the different number of shirt sizes worn.
let shirtSizes = [0, 0, 0, 0, 0, 0, 0];
for (i =0; i < runners.length; i++){
    
    if (runners[i].shirt_size === 'XS'){
        shirtSizes[0]+=1;      
    }
    else if (runners[i].shirt_size === 'S'){
        shirtSizes[1] += 1;      
    }
    else if (runners[i].shirt_size === 'M'){
        shirtSizes[2] += 1;      
    }
    else if (runners[i].shirt_size === 'L'){
        shirtSizes[3] += 1;      
    }
    else if (runners[i].shirt_size === 'XL'){
        shirtSizes[4] += 1;      
    }
    else if (runners[i].shirt_size === '2XL'){
        shirtSizes[5] += 1;      
    }
    else if (runners[i].shirt_size === '3XL'){
        shirtSizes[6] += 1;      
    }
