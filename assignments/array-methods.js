...
let fullName = [];
//Optional syntax
// runners.forEach((item, i) => fullName.push('full_Name: ' + runners[i].first_name + ' ' + runners[i].last_name))
runners.forEach(item => fullName.push('full_Name: ' + item.first_name + ' ' + item.last_name))
...
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners 
//with large sized shirts so they can choose a different size. Return an array named largeShirts that contains 
//information about the runners that have a shirt size of L and log the result

//optional syntax 
//let largeShirts = runners.filter((item, i) => runners[i].shirt_size === 'L')
...
// Problem 1
//Email only list 
let emailList = runners.filter(item => item.email).map(item => item.email);
console.log(emailList)

// Problem 2
//What companies are represented by the runners 
let companiesRepd = [];
runners.forEach(item => companiesRepd.push(item.company_name));
console.log(companiesRepd)

// Problem 3
//tally up the runner id's 
let idTally = runners.reduce((acc, item) => {
    acc = acc + item.id;
    return acc;
}, 0)
