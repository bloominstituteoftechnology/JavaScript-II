...
ticketPriceTotal.push(runners.reduce((a, b) => a + b.donation, 0));
...
// Problem 1
// create an object of names and company names
let namesAndCompanies = {};
namesAndCompanies.name = [];
namesAndCompanies.company = [];
runners.map(runner => {
  namesAndCompanies.name.push(runner.first_name);
  namesAndCompanies.company.push(runner.company_name);
});
console.log(namesAndCompanies);
// Problem 2
// Who donated more than $100
let donatedMoreThan100 = [];
donatedMoreThan100.push(runners.filter(price => price.donation > 100));
console.log(donatedMoreThan100);
// Problem 3
// Highest Donation
let highestDonation = [];
highestDonation.push(Math.max(...runners.map(runner => runner.donation)));
