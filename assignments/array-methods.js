...
{"id":32,"first_name":"Jessey","last_name":"Walhedd","email":"jwalheddvslashdot.org","shirt_size":"L","company_name":"Trilia","donation":5},
...
let allCaps = [];
allCaps = runners.map((item, index, array) => {
...
let largeShirts = runners.filter((item, index, array) => {
...
let ticketPriceTotal = runners.reduce((item, index, array) =>{
...
// Problem 1
//find email for each player

let playerEmail = runners.map((item, index, array) => {
   return item.email;
 });
